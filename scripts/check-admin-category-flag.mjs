#!/usr/bin/env node
/**
 * check-admin-category-flag.mjs — TKT-7.4.A2.
 *
 * Confirms the `admin_category_live` per-area sub-flag exists, defaults
 * to `'placeholder'`, and is overridable via
 * `NEXT_PUBLIC_ADMIN_CATEGORY_LIVE`.
 *
 * Exit codes:
 ...
 *
 * Usage:
 *   node scripts/check-admin-category-flag.mjs
 *   # expect: exit 0 (flag at default)
 *
 *   NEXT_PUBLIC_ADMIN_CATEGORY_LIVE=live node scripts/check-admin-category-flag.mjs
...
const FLAG_NAME = 'admin_category_live';
const DEFAULT_VALUE = 'placeholder';
const ENV_VAR = 'NEXT_PUBLIC_ADMIN_CATEGORY_LIVE';
...
if (!defaultsMatch) {
  console.error('check-admin-category-flag: could not parse FLAG_DEFAULTS');
  process.exit(2);
}
...
if (!flagDefaultMatch) {
  console.error(
    `check-admin-category-flag: '${FLAG_NAME}' not found in FLAG_DEFAULTS`,
  );
  process.exit(2);
}
...
if (defaultValue !== DEFAULT_VALUE) {
  console.error(
    `\ncheck-admin-category-flag: '${FLAG_NAME}' default is '${defaultValue}', expected '${DEFAULT_VALUE}'`,
  );
