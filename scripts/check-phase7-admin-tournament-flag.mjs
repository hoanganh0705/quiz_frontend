#!/usr/bin/env node
/**
 * check-phase7-admin-tournament-flag.mjs — TKT-7.7.A2.
 *
 * Confirms the `phase7_admin_tournament` per-area sub-flag exists,
 * defaults to `'placeholder'`, and is overridable via
 * `NEXT_PUBLIC_PHASE7_ADMIN_TOURNAMENT`.
 *
 * Exit codes:
 ...
 *
 * Usage:
 *   node scripts/check-phase7-admin-tournament-flag.mjs
 *   # expect: exit 0 (flag at default)
 *
 *   NEXT_PUBLIC_PHASE7_ADMIN_TOURNAMENT=live node scripts/check-phase7-admin-tournament-flag.mjs
...
const FLAG_NAME = 'phase7_admin_tournament';
const DEFAULT_VALUE = 'placeholder';
const ENV_VAR = 'NEXT_PUBLIC_PHASE7_ADMIN_TOURNAMENT';
...
if (!defaultsMatch) {
  console.error('check-phase7-admin-tournament-flag: could not parse FLAG_DEFAULTS');
  process.exit(2);
}
...
if (!flagDefaultMatch) {
  console.error(
    `check-phase7-admin-tournament-flag: '${FLAG_NAME}' not found in FLAG_DEFAULTS`,
  );
  process.exit(2);
}
...
if (defaultValue !== DEFAULT_VALUE) {
  console.error(
    `\ncheck-phase7-admin-tournament-flag: '${FLAG_NAME}' default is '${defaultValue}', expected '${DEFAULT_VALUE}'`,
  );
