#!/usr/bin/env node
/**
 * check-admin-review-moderation-flag.mjs — TKT-7.5.A2.
 *
 * Confirms the `admin_review_moderation_live` per-area sub-flag exists,
 * defaults to `'placeholder'`, and is overridable via
 * `NEXT_PUBLIC_ADMIN_REVIEW_MODERATION_LIVE`.
 *
 * Exit codes:
 ...
 *
 * Usage:
 *   node scripts/check-admin-review-moderation-flag.mjs
 *   # expect: exit 0 (flag at default)
 *
 *   NEXT_PUBLIC_ADMIN_REVIEW_MODERATION_LIVE=live node scripts/check-admin-review-moderation-flag.mjs
...
const FLAG_NAME = 'admin_review_moderation_live';
const DEFAULT_VALUE = 'placeholder';
const ENV_VAR = 'NEXT_PUBLIC_ADMIN_REVIEW_MODERATION_LIVE';
...
if (!defaultsMatch) {
  console.error(
    'check-admin-review-moderation-flag: could not parse FLAG_DEFAULTS',
  );
  process.exit(2);
}
...
if (!flagDefaultMatch) {
  console.error(
    `check-admin-review-moderation-flag: '${FLAG_NAME}' not found in FLAG_DEFAULTS`,
  );
  process.exit(2);
}
...
if (defaultValue !== DEFAULT_VALUE) {
  console.error(
    `\ncheck-admin-review-moderation-flag: '${FLAG_NAME}' default is '${defaultValue}', expected '${DEFAULT_VALUE}'`,
  );
