#!/usr/bin/env node
/**
 * check-admin-achievement-flag.mjs — TKT-7.8.A2.
 *
 * Confirms the `admin_achievement_live` per-area sub-flag exists,
 * defaults to `'placeholder'`, and is overridable via
 * `NEXT_PUBLIC_ADMIN_ACHIEVEMENT_LIVE`.
 *
 * Exit codes:
 ...
 *
 * Usage:
 *   node scripts/check-admin-achievement-flag.mjs
 *   # expect: exit 0 (flag at default)
 *
 *   NEXT_PUBLIC_ADMIN_ACHIEVEMENT_LIVE=live node scripts/check-admin-achievement-flag.mjs
...
const FLAG_NAME = 'admin_achievement_live';
const DEFAULT_VALUE = 'placeholder';
const ENV_VAR = 'NEXT_PUBLIC_ADMIN_ACHIEVEMENT_LIVE';
...
if (!defaultsMatch) {
  console.error('check-admin-achievement-flag: could not parse FLAG_DEFAULTS');
  process.exit(2);
}
...
if (!flagDefaultMatch) {
  console.error(
    `check-admin-achievement-flag: '${FLAG_NAME}' not found in FLAG_DEFAULTS`,
  );
  process.exit(2);
}
...
if (defaultValue !== DEFAULT_VALUE) {
  console.error(
    `\ncheck-admin-achievement-flag: '${FLAG_NAME}' default is '${defaultValue}', expected '${DEFAULT_VALUE}'`,
  );
