#!/usr/bin/env node
/**
 * check-admin-tag-flag.mjs — TKT-7.3.A2.
 *
 * Confirms the `admin_tag_live` per-area sub-flag exists, defaults to
 * `'placeholder'`, and is overridable via `NEXT_PUBLIC_ADMIN_TAG_LIVE`.
 *
 * Exit codes:
 ...
 *
 * Usage:
 *   node scripts/check-admin-tag-flag.mjs
 *   # expect: exit 0 (flag at default)
 *
 *   NEXT_PUBLIC_ADMIN_TAG_LIVE=live node scripts/check-admin-tag-flag.mjs
...
const FLAG_NAME = 'admin_tag_live';
const DEFAULT_VALUE = 'placeholder';
const ENV_VAR = 'NEXT_PUBLIC_ADMIN_TAG_LIVE';
...
if (!defaultsMatch) {
  console.error('check-admin-tag-flag: could not parse FLAG_DEFAULTS');
  process.exit(2);
}
...
if (!flagDefaultMatch) {
  console.error(
    `check-admin-tag-flag: '${FLAG_NAME}' not found in FLAG_DEFAULTS`,
  );
  process.exit(2);
}
...
if (defaultValue !== DEFAULT_VALUE) {
  console.error(
    `\ncheck-admin-tag-flag: '${FLAG_NAME}' default is '${defaultValue}', expected '${DEFAULT_VALUE}'`,
  );
