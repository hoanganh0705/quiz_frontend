#!/usr/bin/env node
/**
 * check-phase7-admin-category-flag.mjs — TKT-7.4.A2.
 *
 * Confirms the `phase7_admin_category` per-area sub-flag exists, defaults
 * to `'placeholder'`, and is overridable via
 * `NEXT_PUBLIC_PHASE7_ADMIN_CATEGORY`.
 *
 * Exit codes:
 ...
 *
 * Usage:
 *   node scripts/check-phase7-admin-category-flag.mjs
 *   # expect: exit 0 (flag at default)
 *
 *   NEXT_PUBLIC_PHASE7_ADMIN_CATEGORY=live node scripts/check-phase7-admin-category-flag.mjs
...
const FLAG_NAME = 'phase7_admin_category';
const DEFAULT_VALUE = 'placeholder';
const ENV_VAR = 'NEXT_PUBLIC_PHASE7_ADMIN_CATEGORY';
...
if (!defaultsMatch) {
  console.error('check-phase7-admin-category-flag: could not parse FLAG_DEFAULTS');
  process.exit(2);
}
...
if (!flagDefaultMatch) {
  console.error(
    `check-phase7-admin-category-flag: '${FLAG_NAME}' not found in FLAG_DEFAULTS`,
  );
  process.exit(2);
}
...
if (defaultValue !== DEFAULT_VALUE) {
  console.error(
    `\ncheck-phase7-admin-category-flag: '${FLAG_NAME}' default is '${defaultValue}', expected '${DEFAULT_VALUE}'`,
  );
