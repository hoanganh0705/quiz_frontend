#!/usr/bin/env node
/**
 * phase7-lint-invariants.mjs — Phase 7 admin cross-batch invariant gate.
 *
 ...
 * `phase5-lint-invariants.mjs` and `phase6-lint-invariants.mjs` scripts.
 *
 * ## Checks
 *
 ...
 *      seven sub-flags document `phase7_admin` as a prerequisite.
 ...
 *      (`src/app/admin/**`) is the only consumer of admin services and
 ...
 *      folders, only the admin route group (`src/app/admin/**`) and
 ...
 *      group (`src/app/admin/**`) and admin feature folder
...
// admin route group (`src/app/admin/**`).  Hooks are exempt because
// `usePermission`, `useAdminRole`, `useAdminFeatureFlag`, etc. are
...
function checkAdminFeatureFlagsHaveSubflags() {
  ...
  // Sub-flags must reference `phase7_admin` in their JSDoc. The doc
  ...
  for (const sub of ADMIN_FLAG_NAMES.filter((f) => f !== "phase7_admin")) {
    ...
    if (!window.includes("phase7_admin")) {
      violations.push({
...
function checkAdminImportBoundary() {
  ...
  function isAdminRouteGroup(filePath) {
    // True when the file is under `src/app/admin/**`
    return filePath.includes(`${path.sep}app${path.sep}admin${path.sep}`);
  }
...
async function checkTagAdminSdkBoundary() {
  ...
  for (const file of files) {
    ...
    //   - src/app/admin/** (the admin route group)
