#! /usr/bin/env node
/**
 * check-phase7-admin-ranking-flag.mjs — TKT-7.9.A2.
 *
 * Confirms the `phase7_admin_ranking` per-area sub-flag exists,
 * defaults to `'placeholder'`, and is overridable via
 * `NEXT_PUBLIC_PHASE7_ADMIN_RANKING`.
 *
 * Exit codes:
 ...
 *
 * Usage:
 *   node scripts/check-phase7-admin-ranking-flag.mjs
 *   # expect: exit 0 (flag at default)
 *
 *   NEXT_PUBLIC_PHASE7_ADMIN_RANKING=live node scripts/check-phase7-admin-ranking-flag.mjs
...
const FLAG_NAME = 'phase7_admin_ranking';
const DEFAULT_VALUE = 'placeholder';
const ENV_VAR = 'NEXT_PUBLIC_PHASE7_ADMIN_RANKING';
...
if (!defaultsMatch) {
  console.error('check-phase7-admin-ranking-flag: could not parse FLAG_DEFAULTS');
  process.exit(2);
}
...
if (!flagDefaultMatch) {
  console.error(
    `check-phase7-admin-ranking-flag: '${FLAG_NAME}' not found in FLAG_DEFAULTS`,
  );
  process.exit(2);
}
...
if (defaultValue !== DEFAULT_VALUE) {
  console.error(
    `\ncheck-phase7-admin-ranking-flag: '${FLAG_NAME}' default is '${defaultValue}', expected '${DEFAULT_VALUE}'`,
  );
