/**
 * `<BookmarkButton />` + `<BookmarkButtonErrorNotice />` unit tests.
 *
 * Source epic:   Story 3.10 — Bookmarks add / remove + membership lookup.
 * Source ticket: TKT-3.10.D1.
 *
 * Cases per the ticket AC #1–6:
 *
 *   (a) Filled and outlined icon states match `aria-pressed` and
 *       accessible labels.
 *   (b) Unauthenticated state is disabled with "Sign in to bookmark"
 *       tooltip.
 *   (c) Loading state is neutral, non-clickable, and animated.
 *   (d) Pending state retains the optimistic icon and prevents a
 *       second native click while the hook cooldown owns coalescing.
 *   (e) Icon-only and icon-with-label variants share the same
 *       semantics.
 *   (f) No auth, SWR, wrapper, or generated SDK import exists in the
 *       controlled primitive (verified by an import-graph test in
 *       TKT-3.10.E2 — not in this file).
 *
...
// ---------------------------------------------------------------------------
// (a) Bookmarked + unbookmarked states
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// (b) Unauthenticated — disabled + tooltip
// ---------------------------------------------------------------------------
...
describe('<BookmarkButton /> — loading', () => {
  it('(c1) renders the loading test id and is non-clickable', () => {
    ...
    expect(button).toBeDisabled();
    // The icon SVG should carry the `animate-pulse` class. SVG
    // elements in jsdom expose their `class` attribute via
    // `getAttribute('class')` (the standard DOM string form),
    // whereas `className.baseVal` is also available.
...
// ---------------------------------------------------------------------------
// (d) Pending — keeps optimistic icon, prevents second click
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// (e) Icon-only vs icon-with-label variants
// ---------------------------------------------------------------------------
...
describe('<BookmarkButton /> — variant semantics', () => {
  it('(e1) icon variant has aria-label and no visible text', () => {
    ...
    const button = screen.getByTestId('bookmark-button-bookmarked');
    expect(button).toHaveAttribute('aria-label', 'Remove bookmark');
    // No visible bookmark text node in icon mode.
...
describe('<BookmarkButton /> — keyboard', () => {
  it('(h) the resolved button accepts keyboard activation', () => {
    ...
    const button = screen.getByTestId('bookmark-button-not-bookmarked');
    // `Enter` keypress on a `<button>` triggers click.
    button.focus();
    fireEvent.keyDown(button, { key: 'Enter', code: 'Enter' });
    // We do NOT assert the click fired here — `fireEvent.keyDown`
    // does NOT dispatch a synthetic click in jsdom. The contract
    // under test is that the button is keyboard-focusable and
    // not disabled; the click handler is verified separately.
    expect(button).not.toBeDisabled();
    expect(onToggle).not.toHaveBeenCalled();
  });
});