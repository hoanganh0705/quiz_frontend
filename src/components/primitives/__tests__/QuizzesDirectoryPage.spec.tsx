/**
 * `<QuizzesDirectoryPage />` — unit spec.
 *
 * Source epic: Epic 3.5 — Global quizzes list + filters.
 * Source ticket: TKT-3.5.D1.
 *
 * Locks the four documented cases (ticket §"Testing checklist"):
 *
 *   (a) loading → 20 skeletons.
 *   (b) success → grid + load-more button.
 *   (c) filter-empty (no active filters) → `<QuizGridEmpty />` with
 *       "No published quizzes yet." copy.
 *   (d) filter-no-match (active filters, zero results) → `<QuizGridEmpty />`
 *       with "No quizzes match these filters" copy + "Reset filters" CTA.
 *
 * The test mocks `next/navigation`, the `listQuizzes` / `listTags` SDK
 * wrappers, and the `useQuizzesPopular` / `useQuizzesTrending` /
 * `useCategoriesRanked` / `useTagsPopular` hooks so it can run in
...
// ─── Mocks (set up BEFORE the SUT import) ─────────────────────────────────
...
// `useQuizzesList` is the most involved — mock it with a controllable
// shape so each test can pick loading / success / empty / filter-empty.
...
// ─── (c) filter-empty (no active filters) → "No published quizzes yet." ───
...
// ─── (d) filter-no-match → "No quizzes match these filters" + Reset CTA ───
...
describe('QuizzesDirectoryPage — filter no-match (active filters, zero results)', () => {
  it('renders QuizGridEmpty with the "filters-no-match" variant and a Reset CTA', async () => {
    ...
    // The "Reset filters" CTA must be present in the filter-no-match
    // variant (the directory-empty variant has no CTA).
  ...
  it('clicking Reset filters clears the filter state', async () => {
    ...
    // Wait for the page to render the filter-no-match variant.
    await waitFor(() => {
...
// ─── (e) missing-thumbnail gradient fallback (TKT-3.5.F4) ────────────────
...
describe('QuizzesDirectoryPage — missing-thumbnail gradient fallback', () => {
  it('renders a deterministic gradient on the fallback element when imageUrl is null', () => {
    ...
    // The fallback element must carry a non-empty inline
    // `background:` style — a `linear-gradient(...)` rule that the
    // deterministic helper picked from the palette.
    expect(style).toMatch(/linear-gradient\(/)
    // The data-quiz-id is propagated so the deterministic mapping is
    // auditable in the DOM.
