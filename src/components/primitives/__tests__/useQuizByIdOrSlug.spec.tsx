...
const { getQuizByIdOrSlugMock } = vi.hoisted(() => ({
...
describe('useQuizByIdOrSlug — success', () => {
  it('(B2 AC #1 + #2) returns the projected player-safe quiz when the wrapper resolves', async () => {
    getQuizByIdOrSlugMock.mockResolvedValue(
      makeResponse('quiz-A') as unknown as Awaited<
        ReturnType<typeof getQuizByIdOrSlug>
      >,
    );
  ...
  it('(B2 AC #2) the projection is applied: an isCorrect leak on the wrapper response is stripped', async () => {
    ...
    getQuizByIdOrSlugMock.mockResolvedValue(
      raw as unknown as QuizResponseDto,
    );
...
describe('useQuizByIdOrSlug — malformed envelope', () => {
  it('(B2 AC #5) an empty/malformed envelope becomes a typed error, not a 404', async () => {
    getQuizByIdOrSlugMock.mockReset();
    getQuizByIdOrSlugMock.mockResolvedValue(
      null as unknown as Awaited<ReturnType<typeof getQuizByIdOrSlug>>,
    );

    const { result } = renderHook(() => useQuizByIdOrSlug('quiz-malformed'));

    // Wait until the hook reaches a settled state. The mock
    // returns `null`, the hook's fetcher throws `wrapAsApiError`,
    // and `useSingleWithRetry` exposes the resulting ApiError.
    // Polling is repeated to defeat the React 19 / RTL 16
    // `result.current` stale-snapshot race observed in this file.
    await waitFor(
      () => {
        return result.current.error !== null || result.current.notFound;
      },
      { timeout: 3000, interval: 50 },
    );

    // Diagnostic: how many times was the wrapper called?
    expect(getQuizByIdOrSlugMock).toHaveBeenCalled();
    expect(result.current.notFound).toBe(false);
    // One more poll to ensure the error is settled before assertion.
    await waitFor(() => {
      expect(result.current.error).toBeInstanceOf(ApiError);
    });
  });
