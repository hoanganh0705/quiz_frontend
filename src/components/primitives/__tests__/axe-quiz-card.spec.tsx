/**
 * axe-core audit for the resolved <QuizCard /> primitive.
 *
 * Source story: PHASE_3_EPICS.md → Story 3.1.
 * Source ticket: TKT-3.1.F1.
 *
 * Runs axe-core against a fragment containing the resolved card and
 * asserts no critical or serious violations. The audit covers:
 *   - the resolved-card happy path (with image)
 *   - the no-image edge case (initials fallback)
 *
...
async function auditQuizCard() {
  ...
  // Structural-only audit. Color-contrast is verified manually in
  // F2 (manual dark-mode check) because jsdom does not implement
  // HTMLCanvasElement.getContext and the color-contrast rule needs it.
