 ...
 * Coverage contract:
 *
 *   (a) When `bulkError.length === 0`, the renderer returns null.
 *   (b) When `bulkError.length > 0`, each row renders the index
 *       badge, the title (from `getUserCopy(code).title`), the
 *       message, the field name (when present), and the status.
 *   (c) Clicking "Re-submit failed only" calls `onReSubmitFailed`.
 *   (d) Clicking "Dismiss" calls `onDismiss`.
 *   (e) The list root has `role="alert"`; the rows are
 *       `role="listitem"`.
 *   (f) The "Field" line is omitted when the row has no field.
 *   (g) Pluralisation: 1 row shows "row", 2+ shows "rows".
 */
...
describe('<BulkErrorList />', () => {
  ...
  it('(b) renders one row per bulk error with index, title, message, field, status', () => {
    ...
    // Status badge (row 0 has status 409)
  ...
  it('falls back to a default copy when the code is not a known ErrorCode', () => {
    ...
    // The fallback copy is "Something went wrong" (see error-codes.ts).
    expect(
