 ...
 *
 * Locks the four interaction contracts:
 *
 *   (a) category change → `onChange` called with new `categoryId`.
 *   (b) tag click → `onChange` called with the updated `tagSlugs`
 *       array (add when missing, remove when present).
 *   (c) sort change → `onChange` called with new `sort`.
 *   (d) difficulty change → `onChange` called with new `difficulty`.
 *
 * Test-environment note: the file lives under
 * `src/components/primitives/__tests__/` so vitest's `jsdom` project
 * picks it up (Epic 3.2 / 3.3 / 3.4 precedent).
 */
...
// ─── Type-only check: required props exist ────────────────────────────────
...
describe('FilterBar — prop contract', () => {
  it('accepts the documented props', () => {
    ...
    // The desktop layout is rendered.
