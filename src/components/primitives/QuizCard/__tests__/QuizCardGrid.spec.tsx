/**
 * <QuizCardGrid /> unit tests.
 *
 * Source story: PHASE_3_EPICS.md → Story 3.1.
 * Source ticket: TKT-3.1.C5.
 *
 * Verifies the items-vs-skeleton fallback rule:
 *   - items > 0  → renders resolved cards
 *   - items === 0 AND skeletonCount > 0 → renders skeletons
 *   - both provided → resolved list wins
 *   - neither → empty grid container
 *
 * ResizeObserver-dependent responsive breakpoint verification is out of
 * scope for the unit layer (jsdom doesn't lay out); the Lighthouse CLS
 * evidence in TKT-3.1.C6 captures the runtime layout behaviour.
 */
