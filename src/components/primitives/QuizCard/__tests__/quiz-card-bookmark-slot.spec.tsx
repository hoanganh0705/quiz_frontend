/**
 * `<QuizCard />` — Story 3.10 / TKT-3.10.E1 bookmark integration tests.
 *
 * Source epic:   Story 3.10 — Bookmarks add / remove + membership lookup.
 * Source ticket: TKT-3.10.E1.
 *
 * Cases per the ticket AC #1–5:
 *
 *   (a) Every card renders one bookmark slot with its quizId.
 *   (b) Clicking the bookmark control does not navigate the card
 *       link (parent-link suppression owned by the slot — D4 AC #5).
 *   (c) Clicking elsewhere on the card retains navigation.
 *   (d) The slot is positioned so the card dimensions never change,
 *       regardless of the slot's internal state (Story 3.1
 *       AC #4 — "Lighthouse no CLS").
 *   (e) Existing card props and consumers remain source-compatible
 *       (`bookmarkSlot={null}` overrides suppress the slot).
 *
 * The tests here mock the slot's downstream dependencies (auth,
 * SWR, bookmarks API) following the same patterns as
 * `<BookmarkButtonSlot.spec.tsx>`. We use the real `<BookmarkButtonSlot />`
 * — the unit under test is the card's wiring, not the slot's
 * internals (those are owned by `<BookmarkButtonSlot.spec.tsx>`).
 */
...
describe('<QuizCard /> — bookmark slot wiring', () => {
  it('(a1) renders the resolved bookmark slot once with the quiz id bound', async () => {
    ...
    // The membership Set is empty in this test, so the rendered
    // button matches the unbookmarked branch.
    expect(
  ...
  it('(a2) renders exactly one bookmark slot (no duplicates)', async () => {
    ...
    await waitForSlotHydration(container);
    // Filter by `data-state="resolved"` — the D1 primitive also
    // renders a wrapper with `data-testid="bookmark-button-slot"`
    // but `data-state` derived from the visual state (e.g. "loading",
    // "not-bookmarked"), so the resolved outer wrapper is the only
    // unambiguous match.
...
// ---------------------------------------------------------------------------
// (b) Bookmark click does not navigate the card link
// ---------------------------------------------------------------------------
...
describe('<QuizCard /> — bookmark click does not navigate', () => {
  it('(b1) clicking the bookmark button calls preventDefault on the parent-link click', async () => {
    ...
    // Patch preventDefault on a real MouseEvent instance so we can
    // observe the slot's bubble-phase call. We delegate to the real
    // implementation by capturing a reference to it BEFORE replacing
    // it (the original `Event.prototype.preventDefault` itself is
    // inherited from a chain that includes non-callable prototypes,
    // so we resolve it via `Object.getOwnPropertyDescriptor` to find
    // the actual function).
...
// ---------------------------------------------------------------------------
// (c) Clicking elsewhere on the card still navigates
// ---------------------------------------------------------------------------
...
describe('<QuizCard /> — body click navigates normally', () => {
  it('(c1) clicking the title region (outside the bookmark slot) does not trigger preventDefault', async () => {
    ...
    // The slot's bubble-phase handler is conditional: it only calls
    // preventDefault when the click target is inside the bookmark
    // button. A heading click is OUTSIDE the bookmark button, so the
    // slot wrapper must NOT preventDefault — the parent <a> navigates.
...
// ---------------------------------------------------------------------------
// (d) Card dimensions are stable regardless of bookmark state
// ---------------------------------------------------------------------------
...
describe('<QuizCard /> — stable card dimensions', () => {
  it('(d1) the bookmark slot has `position: absolute` so it claims no flow space', async () => {
    ...
    // The slot renders WITHIN the card, not as a flow sibling. We
    // assert by position: the wrapper sits inside the card.
    const card = screen.getByTestId('quiz-card');
  ...
  it('(d2) authenticated vs unauthenticated render yields the same outer card flow structure', async () => {
    // Render an authenticated card first.
    useAuthStateMock.mockReturnValue({ isAuthenticated: true });
    ...
    // Render an unauthenticated card with the same quiz.
    ...
    // The outer card's data-* attributes (the no-CLS contract) must be
    // identical across auth states. We strip the inner D1 primitive's
    // ever-changing `data-state` so the assertion is not flaky.
    ...
    // The slot wrapper is always present in BOTH auth states. CLS
    // contract: the slot occupies absolute positioning, so the flow
    // siblings of the slot are unchanged.
...
describe('<QuizCard /> — source compatibility', () => {
  it('(e1) bookmarkSlot={null} renders NO bookmark control', () => {
    ...
    // The card is the only mount; no `<BookmarkButtonSlot />` is
    // rendered. We assert the absence of ANY bookmark control
    // (inner D1 primitive div included).
    expect(
