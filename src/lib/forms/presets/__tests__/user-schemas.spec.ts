...
it('accepts a valid full payload', () => {
  const result = updateMyProfileSchema.safeParse({
    ...
    avatarPublicId: '00000000-0000-7000-8000-000000000000/00000000-0000-7000-8000-000000000000',
  });
  expect(result.success).toBe(true);
});
...
it('rejects an invalid avatarPublicId (not a publicId shape)', () => {
  const result = updateMyProfileSchema.safeParse({
    avatarPublicId: 'not-a-public-id-at-all',
  });
  expect(result.success).toBe(false);
});
