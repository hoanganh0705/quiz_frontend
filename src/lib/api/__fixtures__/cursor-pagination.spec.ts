/**
 * Stub test for the cursor-pagination fixture builders.
 *
 * Source epic:   Epic 3.2 — Cursor pagination primitive (`useCursorPaginated`).
 * Source ticket: TKT-3.2.B3 (acceptance criterion #4 — "A stub test in the
 *                same file…passes — the file exports the three builders
 *                and the stub imports them").
 *
 * The substantive test cases (single-page happy path, multi-page round-trip,
 * error fixture) live in the hook spec added in D2 of the same story; this
 * stub exists only to prove the fixture module exports what the spec
 * claims it does, and to keep the test count for the `__fixtures__/`
 * module non-zero so a future "no tests here, delete it" cleanup pass does
 * not remove the file.
 */
...
describe('makeMultiPageCursorResponse', () => {
  it('builds N pages with the last page terminating the cursor', () => {
    ...
    // First two pages have a non-null nextCursor + hasNextPage=true.
    ...
    // Each page's items are unique and well-formed.
