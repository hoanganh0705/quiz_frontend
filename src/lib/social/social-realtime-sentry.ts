/**
 * `social-realtime-sentry.ts` — Phase 6.10 Sentry breadcrumb helpers.
 *
 ...
 * ## Purpose
 *
 * Centralised helpers for the `social:6.10` Sentry breadcrumb category
 ...
 * realtime layer uses to emit breadcrumbs; the `social-lint-invariants`
 ...
 * (`scripts/social-lint-invariants.mjs`, TKT-6.10.G3) fails the build
 ...
 * `social:6.10` so the Sentry dashboard can split the periodic
 ...
 * canonical `social:6.10` so the dashboard can split validation
 ...
 * canonical `social:6.10` so the dashboard can split self-actions
 ...
 * canonical `social:6.10` so the dashboard can split out-of-order
...
export interface SocialRealtimeBreadcrumbData {
  ...
  reason?: string;
  /** The Epic 6.10 version. Defaults to `SOCIAL_EPIC_6_10_VERSION`. */
  epicVersion?: string;
}
...
/**
 * Emit a `social:6.10` breadcrumb for an Epic 6.10 (realtime social)
 * event.
 *
...
/**
 * Emit a `social:6.10:reconnect-reconciliation` breadcrumb for an
 ...
 * intentionally separate from `social:6.10` so the dashboard can split
...
/**
 * Emit a `social:6.10:malformed-payload` breadcrumb when the
 * `validateSocialPayload` runtime validator rejects an event.
 *
 * The sub-category is distinct from the canonical `social:6.10`
...
/**
 * Emit a `social:6.10:self-action-rejection` breadcrumb when the
...
/**
 * Emit a `social:6.10:sequence-guard-drop` breadcrumb when the
