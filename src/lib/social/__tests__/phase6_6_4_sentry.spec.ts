/**
 * `phase6_6_4_sentry.spec.ts` — Locks the Epic 6.4 Sentry
 * breadcrumb contract (TKT-6.4.H1).
 *
 * Coverage:
 *
 *   - `EPIC_6_4_BREADCRUMB_CATEGORY` is exactly "phase6:6.4".
 *   - `EPIC_6_4_VERSION` is exactly "1.0.0".
 *   - `addSocialMutualBreadcrumb` emits the documented payload
 *     shape (route, targetUserId, surface, total?, status?,
 *     durationMs?, code?, epic).
 *   - `addSocialActivityBreadcrumb` emits the documented payload
 *     shape (route, targetUserId, surface, rateLimited?,
 *     cooldownSeconds?, total?, status?, durationMs?, code?,
 *     epic).
 *   - Optional fields are omitted when not provided.
 *   - The breadcrumb category is exactly `phase6:6.4` for every
 *     emitted event.
 *   - `SOCIAL_6_4_ROUTES` documents the three stable SDK route
 *     names.
 *
...
describe("phase6_6_4_sentry — constants", () => {
  it("EPIC_6_4_BREADCRUMB_CATEGORY is exactly 'phase6:6.4'", () => {
    expect(EPIC_6_4_BREADCRUMB_CATEGORY).toBe("phase6:6.4");
  });

  it("EPIC_6_4_VERSION is exactly '1.0.0'", () => {
    expect(EPIC_6_4_VERSION).toBe("1.0.0");
  });
...
describe("phase6_6_4_sentry — addSocialMutualBreadcrumb", () => {
  ...
  it("omits optional fields when not provided", () => {
    ...
    expect(data.epic).toBe(EPIC_6_4_VERSION);
  ...
  it("emits the breadcrumb through the @sentry/nextjs facade with the documented category", () => {
    ...
    expect(mockAddBreadcrumb).toHaveBeenCalledTimes(1);
    const call = mockAddBreadcrumb.mock.calls[0]?.[0] as Record<string, unknown>;
    expect(call.category).toBe("phase6:6.4");
  });
});

describe("phase6_6_4_sentry — addSocialActivityBreadcrumb", () => {
  it("emits the documented payload shape with every documented field", () => {
    addSocialActivityBreadcrumb({
...
describe("phase6_6_4_sentry — breadcrumb contract", () => {
  it("never emits a breadcrumb with a different category", () => {
    ...
    for (const call of mockAddBreadcrumb.mock.calls) {
      ...
      expect(arg.category).toBe("phase6:6.4");
    }
  });
