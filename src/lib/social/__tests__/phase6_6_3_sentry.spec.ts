/**
 * `phase6_6_3_sentry.spec.ts` — Locks the Epic 6.3 Sentry
 * breadcrumb contract (TKT-6.3.H1).
 *
 * Coverage:
 *
 *   - `EPIC_6_3_BREADCRUMB_CATEGORY` is exactly "phase6:6.3".
 *   - `EPIC_6_3_VERSION` is exactly "1.0.0".
 ...
 *   - The breadcrumb category is exactly `phase6:6.3` for every
 *     emitted event.
 *
...
describe("phase6_6_3_sentry — constants", () => {
  it("EPIC_6_3_BREADCRUMB_CATEGORY is exactly 'phase6:6.3'", () => {
    expect(EPIC_6_3_BREADCRUMB_CATEGORY).toBe("phase6:6.3");
  });

  it("EPIC_6_3_VERSION is exactly '1.0.0'", () => {
    expect(EPIC_6_3_VERSION).toBe("1.0.0");
  });
...
describe("phase6_6_3_sentry — addSocialAnalyticsBreadcrumb", () => {
  ...
  it("omits optional fields when not provided", () => {
    ...
    expect(data.epic).toBe(EPIC_6_3_VERSION);
  ...
  it("emits the breadcrumb through the @sentry/nextjs facade with the documented category", () => {
    ...
    expect(mockAddBreadcrumb).toHaveBeenCalledTimes(1);
    const call = mockAddBreadcrumb.mock.calls[0]?.[0] as Record<string, unknown>;
    expect(call.category).toBe("phase6:6.3");
  });
});

describe("phase6_6_3_sentry — addSocialLeaderboardBreadcrumb", () => {
...
describe("phase6_6_3_sentry — breadcrumb contract", () => {
  it("never emits a breadcrumb with a different category", () => {
    ...
    for (const call of mockAddBreadcrumb.mock.calls) {
      ...
      expect(arg.category).toBe("phase6:6.3");
    }
  });
