/**
 * `social-sentry.spec.ts` — Locks the Phase 6 Sentry breadcrumb
 * contract for the social feature (TKT-6.1.H1).
 *
 * Coverage:
 *   - `EPIC_6_1_BREADCRUMB_CATEGORY` is exactly "social:6.1".
 *   - `SOCIAL_EPIC_6_1_VERSION` is exactly "1.0.0".
 ...
 *   - The breadcrumb category is exactly `social:6.1` for every
 *     emitted event.
 *
...
describe("social-sentry — addSocialServiceBreadcrumb", () => {
  ...
  it("emits the breadcrumb through the @sentry/nextjs facade", () => {
    ...
    // and the breadcrumb category is exactly "social:6.1".
    expect(mockAddBreadcrumb).toHaveBeenCalledTimes(1);
    const call = mockAddBreadcrumb.mock.calls[0]?.[0] as Record<string, unknown>;
