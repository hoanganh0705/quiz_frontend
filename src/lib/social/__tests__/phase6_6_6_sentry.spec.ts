/**
 * `phase6_6_6_sentry.spec.ts` — locks the Phase 6.6 Sentry breadcrumb contract
 * (TKT-6.6.G1).
 *
 * Coverage:
 *   - `EPIC_6_6_BREADCRUMB_CATEGORY === "phase6:6.6"`
 *   - `EPIC_6_6_VERSION === "1.0.0"`
 *   - `SOCIAL_6_6_ROUTES.followUser === "social.followUser"`
 *   - `SOCIAL_6_6_ROUTES.unfollowUser === "social.unfollowUser"`
 *   - Success path: all fields logged
 *   - Error path (ApiError): code field present
 *   - Network error (non-HTTP): status field absent
 *   - No secrets logged (followId / friendshipId / token / authorization)
 */
...
// ─── Mock Sentry ─────────────────────────────────────────────────────
// Must use vi.hoisted because phase6_6_6_sentry.ts imports Sentry at
// module level — vi.mock hoists but top-level const in the factory
// (like `const addBreadcrumbMock = vi.fn()`) is still not accessible.
...
describe("phase6_6_6_sentry — TKT-6.6.G1", () => {
  beforeEach(() => {
    addBreadcrumbMock.mockClear();
  });

  describe("exports", () => {
    it('category is "phase6:6.6"', () => {
      expect(EPIC_6_6_BREADCRUMB_CATEGORY).toBe("phase6:6.6");
    });

    it('version is "1.0.0"', () => {
      expect(EPIC_6_6_VERSION).toBe("1.0.0");
    });
  ...
  describe("addFollowMutationBreadcrumb — network error (non-HTTP)", () => {
    it("omits the status field when status is undefined", () => {
      ...
      // status must not be present in the payload
