/**
 * `phase6_6_5_sentry.spec.ts` — Locks the Phase 6.5 Sentry helpers
 * (TKT-6.5.H1).
 *
 * Verifies:
 * - Breadcrumb category is exactly `phase6:6.5`.
 * - Epic version is `1.0.0`.
 * - Helper signatures accept `normalizedQueryLength: number` only (no raw query).
 * - Payload shapes match the documented telemetry contract.
 */
...
} from "@/lib/social/phase6_6_5_sentry";
...
describe("Constants", () => {
  it("EPIC_6_5_BREADCRUMB_CATEGORY is 'phase6:6.5'", () => {
    expect(EPIC_6_5_BREADCRUMB_CATEGORY).toBe("phase6:6.5");
  });

  it("EPIC_6_5_VERSION is '1.0.0'", () => {
    expect(EPIC_6_5_VERSION).toBe("1.0.0");
  });
});
...
describe("addSocialDiscoveryBreadcrumb", () => {
  it("emits breadcrumb with correct category", () => {
    ...
    >;
    expect(breadcrumb.category).toBe("phase6:6.5");
  });
  ...
  it("does NOT accept raw query string (signature test)", () => {
    // This test documents that the helper signature does NOT accept
    // a raw query string — only normalizedQueryLength.
    // If someone tries to call it with query: "secret", TypeScript
    // should reject it at compile time. We verify the shape here.
    addSocialDiscoveryBreadcrumb({
      route: SOCIAL_6_5_ROUTES.getSuggestions,
      kind: "suggestions",
    ...
    const [breadcrumb] = mockAddBreadcrumb.mock.calls[0] as Parameters<
      typeof mockAddBreadcrumb
    >;
    // The only field that relates to query is normalizedQueryLength (a number)
    expect(typeof breadcrumb.data.normalizedQueryLength).toBe("number");
    // No raw query should be present
