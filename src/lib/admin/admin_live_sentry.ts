/**
 * `admin_live_sentry.ts` — Phase 7 admin Sentry breadcrumb helpers.
 *
 ...
 * ## Purpose
 *
 * Centralised helpers for the `admin:7.1` Sentry breadcrumb
 ...
 * `admin-lint-invariants` script (`scripts/admin-lint-invariants.mjs`)
 * asserts that no caller bypasses them via direct
 * `Sentry.addBreadcrumb` calls (TKT-7.1.B6).
 *
 * ## Why a separate file
 *
 * The Phase 6 helpers (`social-sentry.ts`, `social-search-sentry.ts`,
 * `social-block-sentry.ts`) emit `phase6:*` category breadcrumbs.
 * Phase 7 admin uses a distinct `admin:7.1` category so the
...
/**
 * Emit a `admin:7.1` breadcrumb with the documented payload
...
/**
 * Emit a `admin:7.1` breadcrumb that captures the before / after
