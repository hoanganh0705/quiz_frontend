 ...
 *
 * Tests cover every transition from every source state × every event type,
 * plus the exhaustiveness check and boundary conditions.
 */
...
/** Helper: apply an event to a given context and return the new context. */
function transition(
...
/** Helper: build a minimal WsError for ERROR events. */
