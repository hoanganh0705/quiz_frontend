/**
 * Unit tests for `useRealtimeQuery`.
 *
...
describe("useRealtimeQuery", () => {
  it("returns SWR response when swrKey is non-null", async () => {
    ...
    // SWR response has `data` property.
    // The exact value depends on SWR's internal timing.
  ...
  it("deduplicates rules with the same (event, keyToInvalidate) pair", async () => {
    ...
    // Two rules targeting the same event + key.
    const rules = [
    ...
    // Only one listener should be registered for notification:sent.
