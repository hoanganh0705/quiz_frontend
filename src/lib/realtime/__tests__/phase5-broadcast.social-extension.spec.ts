/**
 * Spec for the Epic 6.10 extension of `phase5-broadcast.ts`
 * (TKT-6.10.D3).
 *
 * Source epic:   Epic 6.10 — Realtime Social Notifications and Relationship
 *                Invalidation.
 * Source ticket: TKT-6.10.D3.
 *
 * Covers:
 *
 *   - The two new envelope types (`relationship`, `friend-request`).
 *   - The `postRelationshipInvalidation` / `postFriendRequestInvalidation`
 *     helpers.
 *   - The same-tab filter (`getCurrentTabId()`) is applied unchanged.
 *   - The existing four Phase 5 envelopes (`notification`, `instance`,
 *     `tournament`, `achievement`) still pass through unchanged.
 *
 * `phase5-broadcast.ts` constructs a new `BroadcastChannel` per emit
 * and per subscribe call, and closes the emit-channel in the same
 * synchronous tick. The spec therefore observes wire shapes by
 * intercepting the constructor + postMessage calls (via a tracking
 * proxy that records every payload posted to any channel), rather
 * than by inspecting the registry after the fact.
 */
...
// ─── Recording MockBroadcastChannel ──────────────────────────────────────────
...
class MockBroadcastChannel {
  ...
  postMessage(data: unknown): void {
    ...
    // Dispatch to peers (cross-instance / cross-tab semantics).
...
// ─── Existing four envelopes (regression baseline) ──────────────────────────

describe("phase5-broadcast — existing four envelopes (regression baseline)", () => {
  it("emits a 'notification' envelope with the documented payload shape", () => {
    ...
    expect(recordedPosts[0]!.channelName).toBe("phase5/invalidation");
...
// ─── New 'relationship' envelope ────────────────────────────────────────────

describe("phase5-broadcast — 'relationship' envelope (TKT-6.10.D3)", () => {
...
// ─── New 'friend-request' envelope ──────────────────────────────────────────

describe("phase5-broadcast — 'friend-request' envelope (TKT-6.10.D3)", () => {
...
// ─── Subscribe / same-tab filter ────────────────────────────────────────────

describe("phase5-broadcast — subscribe + same-tab filter", () => {
  it("invokes the subscriber for a cross-tab (different-tabId) message", () => {
    const handler = vi.fn();
    const unsubscribe = subscribeToPhase5Invalidation(handler);

    // Create a peer channel (simulates a sibling tab). The peer
    // posts a foreign-tabId envelope; the subscriber must receive it.
    const peer = new MockBroadcastChannel(PHASE5_INVALIDATION_CHANNEL);
    peer.postMessage({
      type: "relationship",
  ...
  it("does not echo to the same instance (no self-loop)", () => {
    ...
    // The subscriber's own channel posting a message must not
    // echo to itself (real BroadcastChannel does not echo).
...
describe("phase5-broadcast — channel constant", () => {
  it("exports the phase5/invalidation channel name", () => {
    expect(PHASE5_INVALIDATION_CHANNEL).toBe("phase5/invalidation");
  });
  ...
  it("uses the same channel name for all six envelope types (no separate channel per type)", () => {
    ...
    for (const entry of recordedPosts) {
      expect(entry.channelName).toBe("phase5/invalidation");
    }
  });
});