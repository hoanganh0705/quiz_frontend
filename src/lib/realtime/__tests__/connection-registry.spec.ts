 ...
 * Tests cover:
 *
 * (1)  Singleton: `getOrCreateSocket` returns the same socket on repeated calls.
 * (2)  Listener deduplication: registering the same id twice is a no-op.
 * (3)  Per-listener cleanup: `unregisterSocketListener` removes only one handler.
 * (4)  Disposal: `disposeSocket` closes the socket and clears listeners.
 * (5)  Class API parity: `ConnectionRegistry` class methods mirror functions.
 *
 * ## Environment
 *
 * This spec runs in the `jsdom` vitest project (see vitest.config.ts).
 * jsdom provides `window`, so `isBrowser()` returns true and the registry
 * stores connections. The SSR guard (`typeof window === 'undefined'`) is
 * exercised implicitly: if this spec were run in the node project (no
 * window), every function would no-op and all tests would fail — which is
 * the correct signal that the spec belongs in jsdom.
 *
 * ## Mock strategy
 *
 * `connection-registry` uses a `SocketLike` interface, not a concrete import.
 * The factory function is passed by the tests, so no module mocking is needed.
 */
...
/** Generate a unique namespace per test. */
let _counter = 0;
function ns(): string {
