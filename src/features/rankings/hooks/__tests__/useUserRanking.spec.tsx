/**
 * `useUserRanking.spec.tsx` — locks the public user-ranking hook
 * with privacy gating from TKT-5.5.B4.
 *
 ...
 * - Null `userId` fallback (no service call).
 * - Privacy gating via `RANKING_FORBIDDEN` typed error → `isPrivate: true`.
 * - Privacy gating via `RANKING_NOT_FOUND` typed error → `isPrivate: true`.
 * - Public ranking projection when the server returns a known shape.
 */
...
describe("useUserRanking", () => {
  ...
  it("projects a public response to UserRanking", async () => {
    ...
    // The privacy-aware flag is exposed at the hook surface, not on
    // the `ranking` object itself.
    expect(result.current.isPrivate).toBe(false);
  });
});
