/**
 * `FeedErrorState.spec.tsx` — Locks the feed error-state contract
 * (TKT-6.9.F3).
 *
 * Asserts:
 *
 *   - Each documented error code renders the right branch.
 *   - Unknown codes fall back to the generic copy.
 *   - The retry button is disabled while the rate-limit cooldown
 *     is in effect.
 *   - The `onRetry` callback is invoked when the retry button is
 *     clicked.
 *   - The rate-limit countdown decrements every second.
 *   - The privacy branch does not leak the underlying error code
 *     in the user-visible copy.
 */
