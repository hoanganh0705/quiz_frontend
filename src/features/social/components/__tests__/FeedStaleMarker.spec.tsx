/**
 * `FeedStaleMarker.spec.tsx` — Locks the revalidation indicator
 * contract (TKT-6.9.F4).
 *
 * Asserts:
 *
 *   - `isStale === true` renders the "Updating..." badge.
 *   - `isStale === false` renders `null`.
 *   - The root element has `aria-live="polite"`.
 *   - The component does NOT render an aggressive spinner.
 */
...
describe("FeedStaleMarker (TKT-6.9.F4)", () => {
  ...
  it("does NOT render an aggressive spinner (uses a pulse element instead)", () => {
    render(<FeedStaleMarker isStale={true} />);
    // The pulse element is a small dot, not a spinner.
    expect(screen.getByTestId("feed-stale-marker-pulse")).toBeInTheDocument();
    // Assert no spinner role / data-slot is present.
    expect(
