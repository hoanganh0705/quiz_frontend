/**
 * `AnalyticsChart.spec.tsx` — Locks the widget-renderer contract
 * (TKT-6.3.C2).
 *
 * Asserts:
 *
 *   - A widget with `id` in `ANALYTICS_ZERO_WIDGETS` renders nothing
 *     (returns `null`).
 *   - A widget with `id` NOT in `ANALYTICS_ZERO_WIDGETS` renders its
 *     value, label, and accessible description.
 *   - The component NEVER renders `role="alert"` for zero widgets
 *     (absence is silent).
 *   - The component never renders a "0" placeholder for zero widgets.
 */
