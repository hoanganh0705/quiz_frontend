/**
 * `MutualPreview.spec.tsx` — Locks the preview-row contract
 * (TKT-6.4.B1).
 *
 * Asserts:
 *
 *   - `visibility !== 'visible'` → `PrivacyRestrictedNotice` is
 *     rendered; no avatars are rendered.
 *   - `visibility === 'visible'` and empty → the documented
 *     "No mutual friends" / "No mutual followers" copy is
 *     rendered.
 *   - `visibility === 'visible'` and populated → at most
 *     `MUTUAL_PREVIEW_CAP` avatars are rendered, the overflow
 *     indicator equals `mutualCountOverflow(visible.length, total)`,
 *     and the avatars link to `/users/:userId` (no internal-id
 *     leakage).
 *   - The component copes with a missing `total` (overflow
 *     indicator is hidden).
 */
...
describe("MutualPreview — populated branch", () => {
  ...
  it("clamps the overflow to the hard cap", () => {
    ...
    const overflow = screen.getByTestId("mutual-preview-overflow");
    // 500 - 6 = 494 (clamped from 999 - 6 = 993)
