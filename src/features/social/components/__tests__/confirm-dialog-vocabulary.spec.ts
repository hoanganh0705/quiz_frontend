/**
 * `confirm-dialog-vocabulary.spec.ts` — locks the confirm-dialog vocabulary contract.
 *
 * Coverage (TKT-6.6.F1 — unfollow; TKT-6.7.F1 — block / unblock;
 * TKT-6.8.F2 — unfriend / cancel_friend_request):
 *
 *   - Each entry has non-empty title, body, confirmLabel, cancelLabel, dataTestid
 *   - Body mentions the irreversibility language and consequence
 *   - Body does not mention HTTP status codes or error codes
 *   - `getConfirmDialogCopy(action)` returns the matching entry
 *   - `CONFIRM_DIALOGS` is deeply frozen (Object.isFrozen)
 *   - Scaffold references are gone (the Epic 6.8 entries have distinct
 *     copy from the Epic 6.6/6.7 entries)
 */
...
describe("confirm-dialog-vocabulary — TKT-6.6.F1", () => {
  ...
  describe("CONFIRM_DIALOGS", () => {
    ...
    it("does not contain 'scaffold' markers (Epic 6.8 replaced the scaffolds)", () => {
      // Lock the post-Epic-6.8 vocabulary file to have no scaffold markers.
  ...
  describe("CONFIRM_DIALOGS.unfriend (Epic 6.8 / TKT-6.8.F2)", () => {
    ...
    it("has distinct copy from CONFIRM_DIALOGS.unfollow", () => {
      // The scaffold was a direct reference to UNFOLLOW_VOCABULARY; the
      // Epic 6.8 entry is a separately authored frozen object with
      // distinct copy.
