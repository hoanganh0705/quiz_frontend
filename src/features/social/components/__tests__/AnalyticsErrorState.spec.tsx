/**
 * `AnalyticsErrorState.spec.tsx` — Locks the error-state copy
 * (TKT-6.3.C3).
 *
 * Asserts:
 *
 *   - Each documented error code renders code-specific copy.
 *   - 429 / 5xx / generic 4xx render status-specific copy.
 *   - The retry button calls the `onRetry` callback.
 *   - `isStale: true` renders the stale marker; `isStale: false`
 *     does not.
 *   - The error state uses `role="alert"` (it IS an error).
 */
