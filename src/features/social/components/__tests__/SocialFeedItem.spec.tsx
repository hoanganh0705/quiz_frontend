/**
 * `SocialFeedItem.spec.tsx` — Locks the shared row contract
 * (TKT-6.9.E3).
 *
 * Asserts:
 *
 *   - The actor's avatar renders.
 *   - The actor's username renders.
 *   - The item's timestamp renders.
 *   - The `FeedItemRenderer` dispatcher is invoked with the item.
 *   - The `BlockedContentGate` is rendered (the children are
 *     visible when the relationship is non-blocking).
 *   - The row does NOT contain an anchor tag
 *     (`getByRole('link')` returns `null`).
 */
...
describe("SocialFeedItem (TKT-6.9.E3)", () => {
  it("renders the actor's avatar", () => {
    ...
    // The avatar wrapper renders `data-slot="avatar"`. The image
    // and the fallback are slotted inside it. We assert the wrapper
    // is present.
  ...
  it("renders the actor's username", () => {
    const item = makeItem();
    render(<SocialFeedItem item={item} viewerUserId={VIEWER_ID} />);
    // The actor's username appears twice (once in the avatar row,
    // once in the dispatcher output). We assert at least one is
    // present and that the avatar-row username is rendered.
  ...
  it("does NOT emit any analytics payload on mount", () => {
    // The row is intentionally analytics-free. The test asserts
    // there is no `[data-analytics]` attribute on the root.
    const item = makeItem();
    render(<SocialFeedItem item={item} viewerUserId={VIEWER_ID} />);
    const row = screen.getByTestId("social-feed-item");
