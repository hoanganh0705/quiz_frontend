/**
 * `SocialCountsBadge.spec.tsx` — Locks the cross-list counts badge
 * contract (TKT-6.2.F4).
 *
 * Asserts:
 *
 *   - Each count chip renders with the documented values.
 *   - The "Friends" chip is hidden when `canViewFriends === false`.
 *   - The "Updating" indicator renders when `isStale === true`.
 *   - The component returns `null` when counts is null and not
 *     loading.
 *   - The chip links point to the documented list routes.
 */
