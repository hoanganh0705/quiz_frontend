/**
 * `FriendLeaderboardRow.spec.tsx` — Locks the Friend Leaderboard
 * row contract (TKT-6.3.G1).
 *
 * Asserts:
 *
 *   - The `<Link>` href is `/users/${entry.userId}`; the
 *     navigation URL NEVER carries `followId` /
 *     `friendshipId`.
 *   - The analytics payload sent on tap contains `userId` and
 *     `period` only.
 *   - Rank, username, display name, and XP render in the
 *     documented order.
 *   - The optional `onNavigate` callback fires after the
 *     default analytics emission.
 *   - The data attributes carry the canonical rank / userId /
 *     period for testing and analytics.
 */
...
describe("FriendLeaderboardRow — navigation", () => {
  ...
  it("never serialises internal ids into the href or query string", () => {
    const entry = makeEntry();
    // Sanity-check that the entry type does not even expose
    // internal ids today; if a future regression adds them, the
    // href-generation seam must be re-audited.
...
describe("FriendLeaderboardRow — render", () => {
  ...
  it("omits the display name row when displayName is null", () => {
    ...
    // Only one `<span class="text-sm text-muted-foreground">` -
    // the display name should be absent.
