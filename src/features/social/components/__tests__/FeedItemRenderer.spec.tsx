/**
 * `FeedItemRenderer.spec.tsx` — Locks the dispatcher contract
 * (TKT-6.9.E1).
 *
 * Asserts:
 *
 *   - Each documented `SocialFeedItemType` literal routes to the
...
describe("FeedItemRenderer (TKT-6.9.E1)", () => {
  ...
  it("does NOT perform any client-side filtering, ranking, or personalization", () => {
    // The dispatcher must route the item unchanged. This test is
    // covered by the sub-renderer data attributes (e.g. `data-actor-id`)
    // which must reference the original item's actor, not a filtered
    // or ranked substitute.
    const item = makeItem("quiz_completed", {
      type: "quiz_completed",
      quizId: "q1",
      quizSlug: "general-knowledge",
