/**
 * `MutualFollowersPreview.spec.tsx` — Locks the mutual-followers
 * profile-sidebar preview surface contract (TKT-6.4.E1).
 *
 * Asserts the same branches as `MutualFriendsPreview.spec.tsx`,
 * targeted at the mutual-followers endpoint:
 *
 *   - Privacy branch → privacy notice; no avatars.
 *   - Loading branch → `MutualPreviewSkeleton`.
 *   - Visible branch → avatars + overflow chip + "See all" link.
 *   - Empty branch → documented "No mutual followers" copy.
 *   - "See all" link target → `/social/users/:id/mutual-followers`.
 *   - Avatar link href → `/users/:userId` (no internal-id leakage).
 */
