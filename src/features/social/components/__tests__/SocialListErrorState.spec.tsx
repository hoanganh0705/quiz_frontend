/**
 * `SocialListErrorState.spec.tsx` — Locks the error-state component
 * contract (TKT-6.2.C4).
 *
 * Asserts:
 *
 *   - `SOCIAL_USER_NOT_FOUND` renders the "This account is no
 *     longer available" override.
 *   - `SOCIAL_FRIEND_LIST_FORBIDDEN` renders the "Not available"
 *     override.
 *   - `GLOBAL_RATE_LIMITED` renders the rate-limit hint.
 *   - 5xx errors render the "Please try again in a moment" hint.
 *   - `isStale=true` renders the stale marker above the error.
 *   - The retry button invokes `onRetry`.
 */
