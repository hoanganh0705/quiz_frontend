/**
 * `friend-request-error-copy.spec.ts` — Locks the `friend-request-error-copy.ts`
 * registry contract (TKT-6.8.F3).
 *
 * Coverage:
 *
 *   - The registry exports `FRIEND_REQUEST_ERROR_COPY` and
 *     `getFriendRequestErrorCopy`.
 *   - All 10 documented SOCIAL_* / GLOBAL_* codes are present in the
 *     registry (with explicit assertions for the non-idempotent
 *     DELETE terminal-state entries).
 *   - The `NETWORK_ERROR` sentinel is present and retryable.
 *   - Unknown codes return the generic fallback.
 *   - The terminal-state entries (`SOCIAL_FRIEND_REQUEST_NOT_FOUND`,
 *     `SOCIAL_FRIENDSHIP_NOT_FOUND`) document the user-visible outcome
 *     and are non-retryable.
 *   - `SOCIAL_SELF_FRIEND_REQUEST` has the canonical "cannot send to
 *     yourself" description.
 *   - Each entry has a stable `dataTestid` matching the
 *     `friend-request-error.{code}` convention.
 */
...
describe("friend-request-error-copy — TKT-6.8.F3", () => {
  describe("FRIEND_REQUEST_ERROR_COPY registry", () => {
    it("contains SOCIAL_FRIEND_REQUEST_NOT_FOUND (non-idempotent DELETE terminal state)", () => {
      ...
      // The terminal-state copy documents the user-visible outcome.
  ...
  describe("isFriendRequestErrorRetryable", () => {
    ...
    it("returns true for unknown codes (falls back to a retryable generic fallback)", () => {
      // The fallback copy has an action label, so unknown codes are
