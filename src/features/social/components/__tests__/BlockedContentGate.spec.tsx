/**
 * `BlockedContentGate.spec.tsx` — Locks the privacy-gate contract
 * (TKT-6.2.D2 / C1).
 *
 * Asserts:
 *
 *   - Renders `children` when the relationship is non-blocking.
 *   - Renders the default fallback when the relationship is
 *     `'blocked'` or `'blocked_by'`.
 *   - Accepts a custom `fallback` override.
 *   - `relationshipOverride` short-circuits the SWR read so tests
 *     can exercise both branches without mocking SWR.
 */
