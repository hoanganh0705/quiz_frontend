/**
 * `AnalyticsPeriodFilter.spec.tsx` — Locks the period-selector
 * contract (TKT-6.3.F1).
 *
 * Asserts:
 *
 *   - Each of the three options renders.
 *   - The current period (from the URL via `usePeriodFilter`) is
 *     highlighted via `aria-checked` and `data-current`.
 *   - Clicking a different option calls `usePeriodFilter.setPeriod`
 *     (the hook's `router.replace({ scroll: false })` handles the
 *     URL mutation).
 *   - Clicking the current option is a no-op for the data state
 *     but still triggers the contract call.
 *   - `window.scrollTo` is NEVER called (cross-batch invariant
...
beforeEach(() => {
  ...
  // Spy without replacing the real `window.scrollTo` so React
  // Testing Library continues to function.
