/**
 * `MutualFriendsPreview.spec.tsx` — Locks the mutual-friends
 * profile-sidebar preview surface contract (TKT-6.4.E1).
 *
 * Asserts:
 *
 *   - Loading + no cached items + visible hook → `MutualPreviewSkeleton`.
 *   - Privacy branch (`useSocialListVisibility.canViewFriends === false`)
 *     renders the privacy notice; no avatars leak.
 *   - Privacy branch from the hook (`visibility !== 'visible'`)
 *     renders the privacy notice.
 *   - Hook error path renders the privacy notice (the hook returns
 *     `items: [], total: 0, visibility !== 'visible'` for privacy
 *     codes).
 *   - Visible branch renders up to `MUTUAL_PREVIEW_CAP` avatars and
 *     the overflow chip when `total > MUTUAL_PREVIEW_CAP`.
 *   - "See all" link target is `/social/users/:id/mutual-friends`
 *     with `encodeURIComponent(userId)` (no internal-id leakage).
 *   - Empty branch (visible but no rows) renders the documented
 *     "No mutual friends" copy.
 */
...
beforeEach(() => {
  ...
  // Default: visible to viewer (owner / mutual friend).
