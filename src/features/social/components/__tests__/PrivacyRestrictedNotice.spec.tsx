/**
 * `PrivacyRestrictedNotice.spec.tsx` — Locks the privacy-notice
 * contract (TKT-6.2.F1).
 *
 * Asserts:
 *
 *   - Both variants render their documented copy.
 *   - The `resourceKind` is forwarded as a `data-resource-kind`
 *     attribute so e2e tests can assert the right notice mounts
 *     for the right route.
 *   - The component does NOT emit an analytics payload on mount.
 *   - The component renders `role="status"` for live-region
 *     semantics.
 */
