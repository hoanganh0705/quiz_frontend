/**
 * `ActivityStreamItem.spec.tsx` — Locks the type-discriminated
 * renderer contract (TKT-6.4.B2).
 *
 * Asserts:
 *
 *   - Each documented `ActivityItemType` renders its typed
 *     sub-renderer with the documented `data-testid`.
 *   - An unknown discriminator renders the defensive fallback
 *     with the documented `data-testid` (`activity-item-unsupported`)
 *     and emits a `phase6:6.4` breadcrumb with reason
 *     `unknown_discriminator`.
 *   - The discriminated renderer does not crash on unknown
 *     discriminators.
 */
...
describe("ActivityStreamItem — defensive fallback", () => {
  it("renders the documented fallback for an unknown payload discriminator", () => {
    // The `payload.type` is the discriminator. Use a payload
    // whose type is not part of the documented union so the
    // renderer falls through to the defensive fallback.
    const item = makeItem("badge_earned", {
