/**
 * `SocialCountsCard.spec.tsx` — Locks the counts-card contract
 * (TKT-6.3.E2).
 *
 * Asserts:
 *
 *   - Each count chip renders with the documented test id.
 *   - The Friends chip is hidden for non-permitted viewers
 *     (`useSocialListVisibility.canViewFriends === false`).
 *   - `isStale === true` renders the revalidation indicator.
 *   - The component returns `null` when the counts hook has
 *     no data and is not loading.
 *   - The default variant is `'hub'`; the `'badge'` variant
 *     uses a different visual class.
 */
