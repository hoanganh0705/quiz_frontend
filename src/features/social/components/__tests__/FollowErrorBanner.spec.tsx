/**
 * `FollowErrorBanner.spec.tsx` — locks the FollowErrorBanner component contract
 * (TKT-6.6.E4).
 *
 * Coverage:
 *   - error === null → null
 *   - Each documented error code → correct copy (10 codes)
 *   - Retryable codes → retry button rendered
 *   - Non-retryable codes → no retry button
 *   - Retry button click → onRetry() called
 *   - onRetry undefined → no crash, button not rendered
 */
...
describe("FollowErrorBanner — TKT-6.6.E4", () => {
  ...
  describe("retry button", () => {
    ...
    it("does NOT crash when onRetry is omitted (retryable error)", () => {
      // Should render the message without a button.
  ...
  describe("renders for any FollowErrorCode (catches unknown codes gracefully)", () => {
    it("renders without crashing for an unknown GLOBAL_* code", () => {
      // GLOBAL_BAD_REQUEST is in ErrorCode but not in the COPY_TABLE — should not crash.
