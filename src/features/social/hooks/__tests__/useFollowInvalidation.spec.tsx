/**
 * Spec for `useFollowInvalidation` (TKT-6.10.E3).
 *
 * Source epic:   Epic 6.10 — Realtime Social Notifications and Relationship
 *                Invalidation.
 * Source ticket: TKT-6.10.E3.
 *
 * Locks the follow listener hook contract:
 *   - Flag-placeholder fallback.
 *   - Successful dispatch (mutates followers + following + counts).
 *   - Posts a `relationship-invalidation` cross-tab envelope (the
 *     relationship key is authoritative for follow changes).
 *   - Deduplicated / out-of-order events are dropped silently.
 *   - `followId` never appears in any breadcrumb payload or
 *     cross-tab envelope.
 */
...
} from "@/features/social/hooks/useFollowInvalidation";
import {
...
function makeFollowReceivedPayload() {
  return {
    version: 1,
    actorUserId: "11111111-1111-4111-8111-111111111111",
    targetUserId: "22222222-2222-4222-8222-222222222222",
    correlationId: "corr-follow",
    followerUserId: "11111111-1111-4111-8111-111111111111",
    followedAt: "2026-08-07T09:41:00.000Z",
  };
}
...
describe("useFollowInvalidation (TKT-6.10.E3)", () => {
  ...
  it("invalidates followers + following + counts on every accepted event", async () => {
    function Probe() {
      useFollowInvalidation();
      return null;
    }
    ...
    const payload = makeFollowReceivedPayload();
    stub._emit("follow.received", { event: "follow.received", data: payload });
    ...
    expect(keys).toContain("social/v1/followers/22222222-2222-4222-8222-222222222222");
    expect(keys).toContain("social/v1/following/11111111-1111-4111-8111-111111111111");
  ...
  it("drops events whose dedup key is already in the dedup primitive", async () => {
    ...
    const payload = makeFollowReceivedPayload();
    dedup.add(
      `follow.received::${payload.actorUserId}::${payload.targetUserId}::${payload.correlationId}`,
    );
  ...
  it("drops events whose sequence guard returns 'drop'", async () => {
    ...
    sequenceGuard.accept(
      `follow.received::11111111-1111-4111-8111-111111111111::22222222-2222-4222-8222-222222222222`,
  ...
  it("removes the socket listener on unmount", async () => {
    ...
    expect(
      stub.record.offCalls
        ...
        .some((c) => c.event === "follow.received"),
    ).toBe(true);
  });

  it("never includes `followId` or `friendshipId` in any breadcrumb payload", async () => {
    ...
    const { useFollowInvalidation: reimported } = await import(
      "@/features/social/hooks/useFollowInvalidation"
    );
    ...
    stub._emit("follow.received", {
      event: "follow.received",
      data: makeFollowReceivedPayload(),
    });
