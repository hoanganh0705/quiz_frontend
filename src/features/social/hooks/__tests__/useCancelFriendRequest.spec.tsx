/**
 * `useCancelFriendRequest.spec.tsx` — locks the `useCancelFriendRequest`
 * mutation hook contract (TKT-6.8.D3).
 *
 * Coverage:
 *
 *   - Placeholder flag: `cancel` is a no-op
 *   - `canCancelRequest === false`: `cancel()` is a no-op
 *   - `targetUserId === null`: `cancel()` is a no-op
 *   - Defensive empty `friendshipId`: `cancel()` is a no-op
 *   - Server success: `cancelFriendRequest` called with the
 *     `friendshipId`, SWR keys revalidated
 *   - Non-idempotent DELETE terminal state: 404 with
 *     `SOCIAL_FRIEND_REQUEST_NOT_FOUND` → `alreadyCancelled: true`,
 *     `error: null`, cache revalidated
 *   - Server error: error code surfaced, cache NOT revalidated
 *   - Double-click guard
 *   - `friendshipId` hygiene: id never appears in SWR keys
 */
...
describe("useCancelFriendRequest — TKT-6.8.D3", () => {
  ...
  describe("non-idempotent DELETE terminal state", () => {
    it("treats SOCIAL_FRIEND_REQUEST_NOT_FOUND as alreadyCancelled and revalidates the cache", async () => {
      ...
      // The terminal-state flag must be set and no error banner is
      // surfaced.
      expect(result.current.alreadyCancelled).toBe(true);
      expect(result.current.error).toBeNull();
      // The cache is revalidated to converge the UI on the new state.
      expect(mockMutate).toHaveBeenCalledTimes(3);
    });
  });
  ...
  describe("server error", () => {
    it("surfaces SOCIAL_FRIEND_REQUEST_FORBIDDEN as an error code", async () => {
      ...
      // The cache is NOT revalidated on a true error.
