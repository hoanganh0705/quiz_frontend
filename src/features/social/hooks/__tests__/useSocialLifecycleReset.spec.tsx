/**
 * `useSocialLifecycleReset.spec.tsx` — Locks the extended
 * lifecycle-reset contract for Story 6.3 analytics surfaces
 * (TKT-6.3.B5).
 *
 * Asserts:
 *
 *   - A logout transition (auth true → false) on
 *     `/social/me/analytics` calls `periodReset()` exactly once.
 *   - A logout transition on `/social/users/:id/...` calls
 *     `listReset()` exactly once.
 *   - A logout transition on an unrelated route (e.g. `/social`)
 *     does NOT call either reset.
 *   - A login transition does NOT call either reset.
 *   - A no-op auth-state-change event does NOT call either reset.
 *
 * The hook depends on `next/navigation`'s `usePathname`; the suite
 * stubs it via `next/navigation` so the spec runs in a pure-JS
...
describe("useSocialLifecycleReset — no-op branches", () => {
  ...
  it("does not call either reset on a no-op auth-state-change event", () => {
    ...
    // Cookie remains authenticated throughout.
    dispatchAuthStateChange();
