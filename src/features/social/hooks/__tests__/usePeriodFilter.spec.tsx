/**
 * `usePeriodFilter.spec.tsx` — Locks the URL state contract for the
 * Story 6.3 analytics surfaces (TKT-6.3.B4).
 *
 * Asserts:
 *
 *   - Default state: no `?period` param → period is the default
 *     (`'week'`), `isValid` is `false`.
 *   - `?period=month` from URL is exposed as `period === 'month'`,
 *     `isValid` is `true`.
 *   - Unknown period value (e.g. `?period=garbage`) falls back to
 *     the default and reports `isValid === false`.
 *   - `setPeriod('month')` writes `?period=month` to the URL via
 *     `router.replace(next, { scroll: false })`.
 *   - `setPeriod('week')` (the default) removes the `period` URL key
 *     so the canonical URL stays clean.
 *   - `setPeriod(...)` MUST NOT call `window.scrollTo` (cross-batch
 *     invariant: scroll preservation on period change).
 *   - `reset()` removes the `period` URL key.
 *
