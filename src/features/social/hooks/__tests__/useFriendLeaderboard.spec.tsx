/**
 * `useFriendLeaderboard.spec.tsx` — Locks the friend leaderboard
 * read hook contract (TKT-6.3.D3).
 *
 * Asserts:
 *
 *   - `paginationKind === 'offset'` is the second argument to
 *     `useCursorPaginated` (the primitive contract).
 ...
 *     typed `error`.
 *   - `loadMore()` advances the offset via `useCursorPaginated`.
...
describe("useFriendLeaderboard — empty friends", () => {
  it("returns entries: [] and hasMore: false when the backend reports no friends", async () => {
    ...
    // The hook should converge to entries: [], hasMore: false, and
    // no error.
