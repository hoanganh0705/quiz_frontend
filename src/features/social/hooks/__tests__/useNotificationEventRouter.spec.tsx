/**
 * Spec for `useNotificationEventRouter` (TKT-6.10.E6).
 *
 * Source epic:   Epic 6.10 — Realtime Social Notifications and Relationship
 *                Invalidation.
 * Source ticket: TKT-6.10.E6.
 *
 * Locks the notification-router contract:
 *   - Flag-placeholder fallback (no socket connect, no listener).
 *   - `friend_request` kind → invalidates incoming-requests SWR key.
 *   - `follow` kind → invalidates the generic unread-count key.
 *   - `block` kind → invalidates the blocked-users SWR key.
 *   - Unknown / non-social kinds → dropped silently (no `mutateCarefully`).
 *   - Listener is removed on unmount.
 *   - `friendshipId` / `followId` never appear in any breadcrumb.
 *   - The hook handles `data.kind` nested under `data` AND top-level `type`.
 */
