/**
 * `social-list-loaded-channel.spec.ts` — Locks the compatibility
 * shim contract after the TKT-6.2.G1 channel relocation.
 *
 * Asserts:
 *
 *   - `publishSocialListLoaded` posts the documented (D3) payload
 *     shape with `kind`, `userId`, `tabId`, `at`. The D3 userId is
 *     mirror-copied to the canonical `targetUserId` so G1+ consumers
 *     can read either field.
 *   - The channel is a no-op in environments without
 *     `BroadcastChannel`.
