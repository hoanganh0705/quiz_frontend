/**
 * `activity-discriminator.spec.ts` — Locks the activity type
 * discriminator contract (TKT-6.4.A4).
 *
 * Asserts:
 *
 *   - `ActivityItemType` is a closed union whose members are
 *     exactly the documented `SocialFeedItemType` payload types.
 *   - `ACTIVITY_ITEM_TYPES` is non-empty and matches the union in
 *     order.
 *   - `isActivityItemType` returns `true` for every member of
 *     `ACTIVITY_ITEM_TYPES` and `false` for `null`, `undefined`,
 *     non-strings, and arbitrary strings.
 *   - `DEFENSIVE_FALLBACK_TESTID` is `activity-item-unsupported`.
 *   - `ACTIVITY_RATE_LIMIT_ERROR_CODES` contains `GLOBAL_RATE_LIMITED`
 *     and is non-empty.
 *   - `isActivityRateLimitCode` returns `true` for documented
 *     rate-limit codes and `false` otherwise.
 *   - The frozen `ACTIVITY_DISCRIMINATOR_INVARIANTS` record
 *     exposes every constant.
 */
