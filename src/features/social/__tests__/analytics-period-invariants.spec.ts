/**
 * `analytics-period-invariants.spec.ts` — Locks the period
 * discriminator contract (TKT-6.3.A3).
 *
 * Asserts:
 *
 *   - `ANALYTICS_VALID_PERIODS` is exactly `readonly ['week', 'month', 'all']`.
 *   - `ANALYTICS_DEFAULT_PERIOD` is `'week'`.
 *   - `ANALYTICS_PERIOD_URL_PARAM_KEY` is `'period'`.
 *   - `ANALYTICS_FORBIDDEN_SCROLL_RESET_TRIGGERS` includes `'period'`.
 *   - `isAnalyticsPeriod` returns `true` for valid values and `false` otherwise.
 *   - `coerceAnalyticsPeriod` falls back to the default on miss / type-mismatch.
 *   - The `ANALYTICS_PERIOD_INVARIANTS` frozen record exposes every constant.
 */
