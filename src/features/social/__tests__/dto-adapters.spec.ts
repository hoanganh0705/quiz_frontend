/**
 * `dto-adapters.spec.ts` — unit tests for the social DTO adapters
 * (TKT-6.1.C2).
 *
 * The spec covers every acceptance-criteria branch the ticket lists:
 *
 *   - `stripRelationshipInternalIds` discards `followId` /
 *     `friendshipId` and preserves the canonical projection shape.
 *   - `toRelationship` returns the documented enum for known inputs
 *     and `'none'` for any unknown string or malformed wire payload.
 *   - `normalizeSocialPage`, `normalizeSocialOffsetPage`, and
 *     `normalizeSocialCursorPage` discriminate correctly on
 *     `paginationKind` and return the empty offset page for
 *     `null` / `undefined`.
 *   - `normalizeSocialFeedItemPayload` returns `null` for unknown
 *     payload `type` values and narrows correctly for documented
 *     variants.
 *
 * The spec intentionally exercises the "unknown input" branches as
 * loudly as the "happy path" — the adapters are the defensive layer
 * the rest of the story relies on.
 */
...
describe("social/dto-adapters", () => {
  ...
  describe("toRelationship", () => {
    ...
    it("applies the precedence order: friend > blocked > blocked_by > pending > following > follower", () => {
      // `isFriend` beats every other flag.
      expect(
        toRelationship({
          isFriend: true,
          isBlocked: true,
      ...
      // `isBlocked` beats `isBlockedBy` and `hasPendingRequest`.
      expect(
        toRelationship({
          isBlocked: true,
          isBlockedBy: true,
  ...
  describe("toSocialUserSummaryFromFriendRow", () => {
    it("projects a FriendDto into the canonical summary and discards friendshipId", () => {
      ...
      // `id` is aliased to the userId (the rest of the app routes by
      // user id, never by the friendship internal id).
