/**
 * `mutual-count-invariants.spec.ts` — Locks the mutual-count cap
 * contract (TKT-6.4.A3).
 *
 * Asserts:
 *
 ...
 *   - `MUTUAL_TOTAL_HARD_CAP === 500`.
 *   - `mutualCountOverflow(visible, total)` returns `0` when
 *     `total <= visible`; otherwise returns
 *     `min(total - visible, MUTUAL_TOTAL_HARD_CAP - visible)`.
 *   - The frozen `MUTUAL_COUNT_INVARIANTS` record exposes every
 *     constant.
 */
...
describe("mutualCountOverflow", () => {
  ...
  it("clamps to the hard cap when total exceeds MUTUAL_TOTAL_HARD_CAP", () => {
    // 500 - 6 = 494 (max overflow for visible = 6)
    expect(mutualCountOverflow(6, 500)).toBe(494);
    // 999 (backend would never return this, but the helper must
    // clamp to the same value as 500 — the UI must never display
    // an overflow count that exceeds the documented hard cap).
  ...
  it("returns 0 when visible already meets the hard cap", () => {
    // 500 - 500 = 0 (no overflow possible)
