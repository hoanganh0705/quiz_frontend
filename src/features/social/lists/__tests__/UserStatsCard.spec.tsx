/**
 * `UserStatsCard.spec.tsx` — Locks the per-user stats page
 * contract (TKT-6.3.E3).
 *
 * Asserts:
 *
 *   - `visibility !== 'visible'` → `PrivacyRestrictedNotice` is
 *     rendered; no `AnalyticsChart` is rendered.
 *   - `visibility === 'visible'` + `isLoading` + no cached data
 *     → `UserStatsSkeleton` is rendered.
 *   - `visibility === 'visible'` + populated → one
 *     `AnalyticsChart` per non-zero widget renders.
 *   - `staleness !== 'fresh'` → `ConsistencyNotice` rendered.
 *   - Error + not loading → `AnalyticsErrorState` with code-
 *     specific copy and a retry CTA.
 *   - Self branch (viewer IS the target) renders a self
 *     variant with the UserStatsSkeleton.
 *   - Empty branch (zero-widget catalogue hides every chart)
 *     renders `AnalyticsEmptyState` with `kind="stats"`.
 */
...
beforeEach(() => {
  ...
  // Default: a viewer who is NOT the target user.
