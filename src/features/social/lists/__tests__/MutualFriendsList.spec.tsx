/**
 * `MutualFriendsList.spec.tsx` — Locks the mutual-friends full-list
 * page contract (TKT-6.4.E2).
 *
 * Asserts:
 *
 *   - `visibility !== 'visible'` → `PrivacyRestrictedNotice`; no list.
 *   - Loading + no cached items → `MutualListSkeleton`.
 *   - Error + no cached items → `MutualErrorState` with code-specific
 *     copy.
 *   - Visible with no items → `MutualEmptyState` (friends copy).
 *   - Populated → list of `SocialListRow`s with internal-id-free
 *     `href`.
 *   - Pagination footer: rendered when `hasMore === true`, calls
 *     `loadMore()` on click.
 *   - `BlockedContentGate` filters rows when the cached relationship
 *     flips to `'blocked'` / `'blocked_by'` (verified by rendering a
...
beforeEach(() => {
  ...
  // Default: relationship resolves to `'none'` (no block).
...
describe("MutualFriendsList — blocked-content gate", () => {
  it("does not render rows when the cached relationship resolves to 'blocked'", () => {
    // Override the gate to simulate a freshly-revalidated relationship.
    const rows = [makeMutual(1), makeMutual(2)];
    mockUseMutualFriends.mockReturnValue({
      ...PLACEHOLDER_RESULT,
      visibility: "visible",
    ...
    // The `BlockedContentGate` reads `useRelationship` (cached via
    // SWR). We can't override the gate from outside; instead we
    // simulate the post-revalidation state by making `useRelationship`
    // resolve to `'blocked'` and rendering a known blocked user.
    mockUseRelationship.mockReturnValue({
    ...
    render(<MutualFriendsList targetUserId="user-1" />);
    // The fallback panel is rendered in place of children.
