/**
 * `FriendsList.spec.tsx` — Locks the friends-list page contract
 * (TKT-6.2.E3).
 *
 * Asserts:
 *
 *   - Non-owner non-mutual viewer → `PrivacyRestrictedNotice` (and
 *     `useFriends` is NOT called).
 *   - `useFriends` returns `SOCIAL_FRIEND_LIST_FORBIDDEN` →
 *     `PrivacyRestrictedNotice` (not error state).
 *   - Owner / mutual friend → list with documented UX.
 *   - Empty list → `SocialListEmptyState` with the friends copy.
 *   - `STATUS 403` (any code) is treated as a privacy notice.
 */
