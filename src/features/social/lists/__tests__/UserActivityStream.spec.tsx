/**
 * `UserActivityStream.spec.tsx` — Locks the per-user activity
 * stream page contract (TKT-6.4.F1).
 *
 * Asserts the documented branches:
 *
 *   - `visibility !== 'visible'` → `PrivacyRestrictedNotice`; no items.
 *   - `rateLimitedUntil !== null` → `ActivityRateLimitNotice` with
 *     cooldown seconds; no items rendered.
 *   - Loading + no cached items → `ActivitySkeleton`.
 *   - Items.length === 0 + no error → `ActivityEmptyState`.
 *   - Error → `ActivityErrorState` with code-specific copy.
 *   - Rate-limit error code → NOT routed to `ActivityErrorState`.
 *   - Populated → `ConsistencyNotice` (when stale) + list of
 *     `ActivityStreamItem`s; load-more footer when `hasMore` is true.
 *   - `BlockedContentGate` filters cached rows on a relationship flip.
 */
...
describe("UserActivityStream — rate-limit branch", () => {
  it("renders the ActivityRateLimitNotice when rateLimitedUntil is in the future", () => {
    ...
    // The activity items are NOT rendered.
    expect(screen.queryByTestId("user-activity-stream")).toBeNull();
  });
});
...
describe("UserActivityStream — error branch", () => {
  ...
  it("does NOT route the ACTIVITY_RATE_LIMITED error to ActivityErrorState", () => {
    // The activity rate-limit is surfaced via the dedicated
    // `rateLimitedUntil` branch (TKT-6.4.F1 AC #2). When the hook
    // is configured to surface the rate-limit via `error` (without
    // setting `rateLimitedUntil`), the page MUST NOT render
    // `ActivityErrorState`. Instead it falls through to the empty
    // branch.
    mockUseUserActivity.mockReturnValue({
      ...PLACEHOLDER_RESULT,
      visibility: "visible",
