/**
 * `MutualFollowersList.spec.tsx` — Locks the mutual-followers
 * full-list page contract (TKT-6.4.E2).
 *
 * Mirrors `MutualFriendsList.spec.tsx`, targeted at the
 * mutual-followers endpoint. Asserts:
 *
 *   - Privacy branch → `PrivacyRestrictedNotice`.
 *   - Loading branch → `MutualListSkeleton`.
 *   - Error branch → `MutualErrorState`.
 *   - Empty branch → `MutualEmptyState` (followers copy).
 *   - Populated branch → list of `SocialListRow`s; load-more footer.
 *   - `BlockedContentGate` filters rows on a cached relationship flip.
 */
