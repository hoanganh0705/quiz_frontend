/**
 * Spec for `validateSocialPayload` (TKT-6.10.C3).
 *
 * Source epic:   Epic 6.10 — Realtime Social Notifications and Relationship
 *                Invalidation.
 * Source ticket: TKT-6.10.C3.
 *
 * Locks the runtime-validation contract the router and the listener
 * hooks depend on. Covers well-formed payloads, missing / non-UUID
 * ids, self-action rejection, version checks, and the
 * `friend.request.responded` decision check.
 */
