/**
 * Spec for the social-event payload DTOs (TKT-6.10.C2).
 *
 * Source epic:   Epic 6.10 — Realtime Social Notifications and Relationship
 *                Invalidation.
 * Source ticket: TKT-6.10.C2.
 *
 * Locks the wire format the social realtime layer consumes. The
 * `pnpm test` output of this spec is the cross-reference for every
 * later ticket that imports the payload DTOs.
 */
...
describe("social-event-payloads — hygiene invariants", () => {
  ...
  it("every payload type omits `friendshipId` and `followId`", () => {
    // The lint script enforces this at the file level; the spec
    // asserts it via a representative sample to catch any
    // regression that bypasses the lint script.
    const sample = {
      ...buildBase(),
