/**
 * Spec for `realtime-ws-error-copy.ts` (TKT-6.10.F1).
 *
 * Source epic:   Epic 6.10 — Realtime Social Notifications and Relationship
 *                Invalidation.
 * Source ticket: TKT-6.10.F1.
 *
 * Locks the WS-error user-copy registry contract:
 *   - All four documented WS error codes are present.
 *   - Each entry has `title`, `description`, optional `actionLabel`,
 *     and `dataTestid`.
 *   - None of the copy mentions `ws://`, `socket`, or `handshake`.
 *   - Unknown codes resolve to the `WS_INTERNAL` fallback.
 *   - The `mapWsErrorToRealtimeCode` helper maps Phase 5 codes to
 *     Epic 6.10 codes; unrecognised codes fall back to `WS_INTERNAL`.
 */
