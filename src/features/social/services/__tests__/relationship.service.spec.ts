/**
 * `relationship.service.spec.ts` — Locks the relationship service contract
 * (TKT-6.1.E1).
 *
 * Coverage:
 *   - Happy path: SDK envelope unwraps, internal ids are stripped,
 *     projection returned.
 *   - 4xx / 5xx: `ApiError` rethrown with `code` and `status` accessible.
 *   - Missing envelope (`data === undefined`): `GLOBAL_INTERNAL_ERROR`.
 *   - Internal-id leakage: any `followId` / `friendshipId` value present in
 *     the wire body is discarded before the service returns.
 */
  ...
  it("getRelationshipStatus drops followId / friendshipId when they leak through", async () => {
    mockSocialControllerGetRelationshipStatus.mockResolvedValue({
      data: {
        ...
        isBlocked: false,
        isBlockedBy: false,
        // The backend may leak these (Phase 6 Risks line 54). The
        // projection type forbids them so we cast through unknown.
        ...({
    ...
    // The result's followId / friendshipId values must be undefined
    // even if the wire payload leaked them. The projection types them
    // as `never` so consumers can never read them.
