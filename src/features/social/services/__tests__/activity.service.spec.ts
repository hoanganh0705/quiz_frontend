/**
 * `activity.service.spec.ts` — Locks the Story 6.4 activity
 * service wrapper contract (TKT-6.4.D1).
 *
 * Asserts:
 *
 ...
 *     documented `code` accessible.
 *   - A rate-limit error (`ACTIVITY_RATE_LIMITED` /
 *     `GLOBAL_RATE_LIMITED`) is decoded; the breadcrumb carries
 *     `rateLimited: true` and the `cooldownSeconds` payload.
 *   - A missing envelope (`null`) throws `GLOBAL_INTERNAL_ERROR`.
 *   - Unknown `type` discriminators are dropped before the
 *     wrapper returns.
 */
...
describe("activity.service — getUserActivity", () => {
  ...
  it("drops items with unknown type discriminators", async () => {
    mockSocialControllerGetUserActivity.mockResolvedValue({
      data: [
        ...
        {
          id: "activity-2",
          // Unknown discriminator — should be dropped by the
          // adapter before the wrapper returns.
          type: "unknown_type",
  ...
  it("decodes rate-limit cooldown from extensions.retryAfterMs on ACTIVITY_RATE_LIMITED", async () => {
    ...
    // The breadcrumb payload should carry the rate-limit signal
    // (one in-flight breadcrumb + one resolved breadcrumb).
    expect(addBreadcrumbMock).toHaveBeenCalledTimes(2);
