/**
 * `discovery.service.spec.ts` — Locks the Story 6.5 discovery
 * service wrapper contract (TKT-6.5.C1).
 *
 * Asserts:
 *
 ...
 *   - `getSearchSuggestions` groups raw strings by kind discriminator
 *     and routes unknown values to `unsupported`.
 *   - `getSearchSuggestions` emits `route: 'social.getSearchSuggestions'`.
 *   - `getSuggestions` emits `route: 'social.getSuggestions'`.
 *   - `getTrendingUsers` emits `route: 'social.getTrendingUsers'`.
 */
