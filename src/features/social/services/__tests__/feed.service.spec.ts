/**
 * `feed.service.spec.ts` — Locks the Story 6.9 feed service wrapper
 * contract (TKT-6.9.C1).
 *
 * Asserts:
 *
 ...
 *     `addSocialFeedBreadcrumb` helper (TKT-6.9.H1).
 *   - 4xx / 5xx errors propagate as `ApiError` with the documented
 *     `code` accessible (the hook branches on `error.code`).
 *   - A rate-limit error (`GLOBAL_RATE_LIMITED` with
 *     `extensions.retryAfterMs`) is decoded; the service does NOT
 *     throw — it returns the documented empty page shape with
 *     `cooldownSeconds` populated.
 *   - A missing envelope (`null`) throws `GLOBAL_INTERNAL_ERROR`.
 *   - Unknown `type` discriminators are dropped before the wrapper
 *     returns.
 *   - The service does NOT construct a cursor client-side; the
 *     `nextCursor` returned is identical to the server response
...
describe("feed.service — getFeed", () => {
  ...
  it("forwards nextCursor and hasMore unchanged from the server response", async () => {
    ...
    // The wrapper must NOT mutate the opaque cursor string.
  ...
  it("drops items with unknown type discriminators", async () => {
    mockSocialControllerGetFeed.mockResolvedValue({
      data: [
        ...
        {
          id: "feed-2",
          // Unknown discriminator — should be dropped by `toFeedItem`
  ...
  it("propagates a 429 GLOBAL_RATE_LIMITED ApiError", async () => {
    ...
    // The service rethrows rate-limit errors so the consumer
    // hook can decode the cooldown via `decodeRateLimit` —
    // mirroring the canonical `activity.service.ts` pattern.
    await expect(getFeed()).rejects.toMatchObject({
