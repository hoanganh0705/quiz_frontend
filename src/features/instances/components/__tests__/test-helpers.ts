/**
 * `test-helpers.ts` — shared mocks for instance component tests.
 *
 ...
 *
 * Provides:
 * - factory for a mocked instance hooks module
 * - factory for a sample InstanceDetail / InstancePlayer
 * - factory for a typed ApiError-like object
 *
 * The test files import this module via `vi.mock` to keep the wiring
 * consistent across the ten integration tests in `components/__tests__/`,
 * `pages/__tests__/`, and `app/instances/[id]/__tests__/`.
 */
...
// Reference INSTANCE_LIFECYCLE_ERROR_CODES so the lint invariant that
// tracks exported types from instance.types.ts stays green.
