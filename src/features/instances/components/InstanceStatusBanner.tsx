...
/**
 * `InstanceStatusBanner` — server-driven instance status banner.
 *
 * Source epic:   Epic 5.1 — SDK coverage & realtime contract foundation.
 * Source story:  5.7 — Instance lobby, authenticated room, and host
 *                lifecycle controls.
 * Source ticket: TKT-5.7.D2.
 *
 * ## What this component owns
 *
 * - Display the instance status string, sourced from
 *   `useInstance` for the initial render.
 * - Reconcile to the realtime store's `instanceStatus` hint when a
 *   newer `eventSequence` arrives; never let a stale `eventSequence`
 *   override the current status.
 * - Render the closed state when the status is `'closed'` or
 *   `'finished'`, or when the realtime store surfaces a cancelled
 *   lifecycle event.
 * - Never contain a timer, a countdown, or any client-side status
 *   advancement. Status transitions are server-driven.
 *
 * ## What this component does NOT do
 *
 * - It does NOT decide permissions; that lives in
 *   `useInstancePermissions` (TKT-5.7.B3).
 * - It does NOT emit socket events; that lives in
 *   `useInstanceSocket` (TKT-5.7.B5).
 */
...
export function InstanceStatusBanner({
  ...
  // Mount the realtime bridge so the store mirror reflects lifecycle
  // events. The hook is idempotent and a no-op when `instanceId === null`.
  ...
  // Reconcile REST vs realtime. The realtime store already drops
  // stale events (the store's `applyLifecycleEvent` returns the
  // unchanged state when `eventSequence <= lastEventSequence`), so a
  // non-null realtime status is by definition newer than what we
  // have on the REST detail. We still gate on `realtimeSequence > 0`
  // because an entry that has only seen player join / leave events
  // has no status hint to apply.
  ...
  if (resolved.status === null) {
    // Still loading or no data yet — render a quiet placeholder so
    // the layout does not shift when the status arrives.
    return (
      <div
