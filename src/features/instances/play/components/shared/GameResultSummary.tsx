...
/**
 * `GameResultSummary` — post-game result summary view.
 *
 ...
 *
 * Renders after `instance_closed` when a `FinalLeaderboardDto` is
 * available. Displays the final leaderboard with the current player's
 * rank highlighted, total score, and a congratulatory or encouraging
 * message based on the player's final position.
 *
 * Mirrors the pattern in `TournamentLeaderboard` (Epic 5.2).
 *
 * The component is purely presentational; no timers or state transitions.
 */
...
interface GameResultSummaryProps {
  /** The final leaderboard from `useLiveLeaderboard().final`. */
  finalLeaderboard: FinalLeaderboardDto | null;
  /** The player's progress from `useInstanceLifecycle().progress`. */
  playerProgress: PlayerProgressDto | null;
  /** Current player's ID (from auth context). */
