/**
 * `instanceRealtime.store.spec.ts` — locks the per-instance realtime
 * store.
 *
 ...
 *
 * Tests cover:
 * - applyPlayerJoined: idempotent by `playerId`, drops stale sequences
 * - applyPlayerLeft: removes the player, drops stale sequences
 * - applyLifecycleEvent: instanceStatus and closedAt derivation
 * - reset(instanceId): clears a single entry
 * - resetAll(): clears every entry
 * - selectors return safe defaults for unknown instances
 */
...
describe("instanceRealtime.store", () => {
  ...
  describe("applyPlayerLeft", () => {
    ...
    it("creates an empty entry for an unknown instanceId", () => {
      ...
      // applyPlayerLeft always touches the entry via `ensureEntry`.
      // The result is an entry with no players.
