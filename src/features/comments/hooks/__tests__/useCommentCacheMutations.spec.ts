/**
 * `useCommentCacheMutations.spec.ts` — unit tests for direct SWR cache mutations.
 *
 ...
 * ## Coverage contract
 *
 *   - applyCommentCreated / applyCommentEdited / applyCommentDeleted /
 *     applyCommentHidden / applyCommentRestored / applyVoteCast /
 *     applyVoteRemoved handle missing snapshots gracefully (no-ops).
 *   - The functions handle missing cache entries gracefully.
 *   - The `useCommentCacheMutations` hook returns register functions.
 */
...
describe("applyCommentCreated", () => {
  it("does nothing when snapshot is missing", () => {
    ...
    // Should not throw - snapshot is required
