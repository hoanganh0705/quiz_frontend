/**
 * `useQuizComments.spec.tsx` — unit tests for the read comments hook.
 *
 * Source epic:   Epic 4.12 — Comments on a quiz.
 * Source ticket: T-4.12.5.
 *
 * ## Coverage contract
 *
 *   - Top-level mode (parentId undefined): calls listQuizComments without
 *     a parent filter; default limit is 20.
 *   - Reply mode (parentId set): calls listQuizComments with the parent
 *     filter; default limit is 50.
 *   - Hook disabled (quizId: null): no fetcher runs; items is empty.
 *   - Successful response populates `items` with `id`-aliased entries.
 *   - 5xx surfaces as `error: ApiError`; 404 still surfaces as an error
 *     (caller can branch on `.code`).
 *   - `hasMore: true` when `meta.pagination.hasNextPage` is true.
 *   - Cursor is forwarded to the service when set.
 *   - SWR key includes the comment discriminator and parentId.
 */
