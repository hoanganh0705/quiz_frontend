/**
 * `useEditReview.spec.tsx` — unit tests for the own-review edit hook.
 *
 ...
 * Coverage contract:
 *
 *   - Correct quizId + payload reach the service.
 *   - Success invalidates list + my-review + eligibility keys.
 *   - 403 REVIEW_FORBIDDEN produces `forbidden` and does NOT invalidate.
 *   - 422 REVIEW_VALIDATION produces `validation`.
 *   - 404 GLOBAL_NOT_FOUND produces `stale` and invalidates.
 *   - 429 / 5xx produce `reverted`.
 *   - Pending state blocks a duplicate update.
 *   - `reset()` clears `error` and `lastOutcome`.
 */
