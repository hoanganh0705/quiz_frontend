/**
 * `useQuizReviews.spec.tsx` — unit tests for the read reviews hook.
 *
 * Source epic:   Epic 4.13.
 * Source ticket: T-4.13.4.
 *
 * Coverage contract mirrors the T-4.12.5 read-hook spec:
 *
 *   - Default mode (no filters): calls listQuizReviews with the
 *     `REVIEWS_DEFAULT_LIMIT` and no cursor.
 *   - Hook disabled (quizId: null): no fetcher runs; items is empty.
 *   - Successful response populates `items` with `id`-aliased entries.
 *   - 5xx surfaces as `error: ApiError`.
 *   - `hasMore: true` when `meta.pagination.hasNextPage` is true.
 *   - Cursor is forwarded to the service when set.
 *
 * The spec lives in the jsdom project because `useCursorPaginated`
 * uses `useSWRInfinite`, which requires a DOM environment.
 */
