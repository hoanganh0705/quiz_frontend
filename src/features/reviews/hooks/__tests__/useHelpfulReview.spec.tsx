/**
 * `useHelpfulReview.spec.tsx` — unit tests for the optimistic helpful
 * toggle hook.
 *
 ...
 * Coverage contract:
 *
 *   - Mark path increments the helpful count by exactly 1 across
 *     every cached page for the quiz.
 *   - Unmark path decrements the helpful count by exactly 1.
 *   - The count never becomes negative (clamps at 0).
 *   - The viewer-marked boolean flips synchronously before the
 *     network promise settles.
 *   - Failure restores the previous viewer state AND revalidates
 *     the cached pages.
 *   - REVIEW_FORBIDDEN (self-review defensive) rolls back safely.
 *   - 429 and 5xx preserve retry capability via `lastError`.
 *   - Rapid clicks within 500 ms produce at most one in-flight
 *     request (cooldown discipline).
 */
...
// SWR's default cache is a `new Map()` singleton (see
// `swr/_internal`). We can read it directly so the tests can
// verify optimistic updates that landed via the helper's
// `globalMutate(predicate, updater)` calls.
...
/**
 * Seed SWR's cache with two pages of reviews for the quiz, both
 * containing the same reviewId (with different helpfulCount
 * states — the second page is the canonical "load more" page).
 *
 * We store entries directly in the default cache Map because the
 * helper's optimistic-update predicate relies on `_k` being
 * present on each entry — `globalMutate(key, value)` does NOT
 * set `_k`, only `useSWR` does. Writing `{ data, _k: key }` to
 * the cache mimics what `useSWR` would have produced.
 */
...
/**
 * Read a cached review list page directly from the SWR cache. We
 * probe the default `SWRConfig` cache via a small React component
 * — `globalMutate(key, …)` does not expose the cached value, only
 * the mutator signature, so we read from the cache directly.
 */
/**
 * Read a cached review list page from SWR's default cache. The
 * cache is a Map singleton (per `const [cache] = initCache(new Map())`
 * in SWR's internals). Each entry is keyed by the serialized key
 * — SWR uses `unstable_serialize(key)` to normalize.
 */
...
beforeEach(() => {
  vi.clearAllMocks();
  // Reset SWR's default cache between tests so a stale `data`
  // entry from the previous case does not bleed into the next.
...
describe('useHelpfulReview — unmark path', () => {
  ...
  it('decrements helpfulCount but clamps at 0', async () => {
    ...
    // The count must clamp at 0 — never go negative.
...
describe('useHelpfulReview — cooldown', () => {
  it('rapid clicks within 500 ms produce at most one in-flight request', async () => {
    ...
    // Fire two toggles back-to-back. The second is dropped by the
    // 500 ms cooldown.
