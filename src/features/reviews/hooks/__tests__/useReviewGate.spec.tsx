/**
 * `useReviewGate.spec.tsx` — unit tests for the deterministic gate
 * resolution hook.
 *
 ...
 *
 * Coverage contract:
 *  - `bootstrapState` unresolved → `loading`.
 *  - `unauthenticated` → `unauthenticated` (no private queries fired).
 *  - Existing my-review wins over attempt-query state.
 *  - No my-review + completed attempt → `eligible`.
 *  - No my-review + no completed attempt → `attempt-required`.
 *  - My-review query failure → `error`, NOT `attempt-required`.
 *  - Eligibility query failure → `error`, NOT `attempt-required`.
...
// `mutate` via a module mock so the hook's `revalidate()` calls
// land on a spy we control.
...
// ─── Tests: discriminated-union branches ─────────────────────────────────────
...
describe('useReviewGate — branch resolution', () => {
  ...
  it('renders `unauthenticated` without firing private queries', () => {
    ...
    // The underlying hooks should never be called in this branch
    // (the consumer rendering the gate keeps their params disabled
    // until `bootstrapState === 'authenticated'`). The hook's
    // own resolution must still yield `unauthenticated`.
    setMyReview({ review: null, hasResolved: false, error: null });
    setAttempt({ hasCompletedAttempt: false, hasResolved: false, error: null });
  ...
  it('renders `existing-review` when my-review has resolved with a review, regardless of attempt state', () => {
    ...
    // Even if the attempt query resolves true/false, the review
    // wins — the attempt hook should not be consulted when a
    // review exists.
    setAttempt({
...
describe('useReviewGate — error classification', () => {
  ...
  it('prefers the my-review error over the eligibility error when both fail', () => {
    ...
    expect(result.current.state.kind).toBe('error');
    if (result.current.state.kind === 'error') {
      // Documented ordering — my-review is the cheaper input;
      // its failure is the more localised signal.
      expect(result.current.state.error).toBe(myError);
    }
  });
