/**
 * `ReviewEditInline.spec.tsx` — RTL tests for the owner-only edit +
 * typed-delete controls.
 *
 * Source epic:   Epic 4.13.
 * Source ticket: T-4.13.16.
 *
 * Verifies:
 *  - Edit / Delete actions render only for the owner.
 *  - Edit mode is prefilled with the current rating and text.
 *  - Cancel restores the persisted values.
 *  - Save enforces rating 1–5 and text 1–2000, then updates once.
 *  - Delete opens an accessible typed-confirm dialog.
 *  - The dialog copy mentions the destructive consequence.
 *  - Typed-confirm is required before the Delete button enables.
 *  - Confirm calls the delete hook exactly once.
 *  - Cancel calls none.
 *  - 404 / forbidden close the editor and refresh the parent.
 *  - 429 / 5xx retain a retry path.
 */
...
describe('ReviewEditInline — edit mode', () => {
  it('prefills with the persisted rating and text', () => {
    ...
    // Rating prefilled → the "5 stars" radio is the checked one.
  ...
  it('drops the edit mode on stale outcome', () => {
    ...
    fireEvent.click(screen.getByTestId(`review-edit-open-${REVIEW_ID}`));
    // Switch `lastOutcome` after render — simulate the hook's
    // outcome arriving mid-edit.
    setEdit({
      update: vi.fn().mockResolvedValue(false),
      lastOutcome: { kind: 'stale', cause: null },
    });
...
describe('ReviewEditInline — typed delete', () => {
  it('opens a typed-confirm dialog with destructive copy', () => {
    ...
    // The body must mention the destructive consequence: rating +
    // helpful counts being removed.
  ...
  it('notifies the parent after a successful delete', async () => {
    ...
    // Outcome lands on the next render — switch the mock so the
    // component picks up the success outcome and fires the
    // `onDeleted` callback from its useEffect.
    setDelete({
      remove,
...
describe('ReviewEditInline — stale / not-found recovery', () => {
  it('closes edit + notifies parent when the delete hook returns not-found', async () => {
    ...
    // Outcome lands on the next render.
    setDelete({
      remove,
