 ...
 * Source ticket: TKT-2.6.T11.
 *
 * ## Design
 *
 * This component renders a Google-branded sign-in button that matches Google's
 * brand guidelines. The button follows the OAuth branding guidelines from
 * https://developers.google.com/identity/branding-guidelines.
 *
 * The button is:
 *   - Hidden when `isAvailable` is `false` (Google Auth not configured).
 *   - Disabled when `disabled` is `true` (during any pending state).
 *   - Shows a loading spinner when `isLoading` is `true` (exchange phase).
 *
 * ## Google Brand Guidelines Compliance
 *
 * - Uses the official Google "G" logo SVG.
 * - Uses the official Google brand colors.
 * - Uses the approved "Continue with Google" text.
 ...
 * This is the official Google logo from their brand guidelines.
 */
...
export const GoogleSignInButton = memo(function GoogleSignInButton({
  ...
  // Hide the button entirely when Google Auth is not configured.
  if (!isAvailable) {
    return null;
  }
