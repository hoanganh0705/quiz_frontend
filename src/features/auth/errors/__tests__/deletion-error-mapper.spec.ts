/**
 * Unit tests for `mapDeletionError` and its type guards.
 *
 ...
 *
 *   1. Invalid password, conflict, validation, terminal auth,
 *      uncertain network, 429, 5xx, and unknown cases are covered.
 *   2. Every failure maps to a non-success state.
 *   3. Conflict and invalid password remain distinct.
 *   4. Mapper purity is guarded by absence of browser/auth side
 *      effects (no `Date.now`, no `Math.random`, no `console`, no
 *      network — verified by reading the test file's setup).
 *
 * ## Strategy
 *
 * The mapper is a pure function. We test it directly with
 * literal inputs. No mocking, no React rendering, no fs.
 *
 * The branch-precedence tests at the bottom act as regression
 * guards: a code-driven branch (e.g. `AUTH_INVALID_TOKEN`) MUST
 * win over a status-driven branch (e.g. 500), and a status-driven
 * branch wins over the final `'uncertain'` fallback.
 *
 * The tests intentionally do NOT assert anything about the
 * `error.message` field. The mapper's anti-leak contract (2.10.T3)
 * forbids rendering raw backend messages; asserting on them
 * would encourage a regression in the UI.
 */
...
// ─── T25.1: AUTH_INVALID_CURRENT_PASSWORD ─────────────────────────────────────
...
describe('mapDeletionError — AUTH_INVALID_CURRENT_PASSWORD', () => {
  ...
  it('preserves status even when the code is the discriminator', () => {
    // The status is forwarded for logging even though
    // `invalid_current` is a code-driven branch.
    const result = mapDeletionError({
      code: AUTH_INVALID_CURRENT_PASSWORD,
      status: 401,
    });
  ...
  it('does NOT classify as "conflict" — the two branches must remain distinct', () => {
    // Regression guard: a code-driven branch win over a
    // 409 status. The mapper must NOT collapse
    // `AUTH_INVALID_CURRENT_PASSWORD` into the conflict flow.
    const result = mapDeletionError({
      code: AUTH_INVALID_CURRENT_PASSWORD,
      status: 409,
    });
...
describe('mapDeletionError — AUTH_INVALID_TOKEN', () => {
  ...
  it('AUTH_INVALID_TOKEN wins over retryable status', () => {
    // Regression guard: the code-driven branch takes precedence
    // over the status fallback, even at 503.
    const result = mapDeletionError({
      code: AUTH_INVALID_TOKEN,
      status: 503,
    });
...
// ─── T25.6: GLOBAL_VALIDATION_FAILED ──────────────────────────────────────────
...
describe('mapDeletionError — GLOBAL_VALIDATION_FAILED', () => {
  ...
  it('preserves validationMessages=[] explicitly', () => {
    // An empty array is a valid input (the backend emitted no
    // field-level messages) and must NOT be replaced by `null`
    // or `undefined`.
    const result = mapDeletionError({
      code: GLOBAL_VALIDATION_FAILED,
      status: 400,
...
// ─── T25.7: retryable status — 0 / 429 / 5xx ─────────────────────────────────
...
describe('mapDeletionError — retryable / uncertain status codes', () => {
  ...
  it('preserves the original code on "uncertain"', () => {
    // The mapper must forward the original `code` so the hook
    // can log it; the mapper never invents a code.
    const result = mapDeletionError({
  ...
  it('does NOT classify 4xx as "uncertain" via status fallback', () => {
    // 4xx is a code-driven branch (or final fallback), NOT a
    // status-driven branch. The mapper must not collapse
    // 4xx into "uncertain" because the hook calls
    // `revalidateAccountExists()` on uncertain, which is the
    // wrong action for a deterministic 401.
    const result = mapDeletionError({
      code: 'SOMETHING_NEW',
      status: 401,
    });
    expect(result.kind).toBe('uncertain'); // final fallback
    expect(result.kind).not.toBe('auth_terminal');
  });
});

// ─── T25.8: unknown code fallback ─────────────────────────────────────────────
...
describe('mapDeletionError — unknown code (final fallback)', () => {
  it('classifies an unknown code with non-retryable status as "uncertain"', () => {
    // The final fallback bucket is the same `'uncertain'` kind
    // the status fallback uses. There is no separate "unknown"
    // kind — the mapper is intentionally conservative.
    const result = mapDeletionError({
      code: 'SOMETHING_NEW',
      status: 418,
    });
  ...
  it('classifies an unknown code with status 200 as "uncertain"', () => {
    // A success status is impossible for a 2xx to be a deletion
    // failure in practice, but the mapper does not assume
    // status semantics — it defers to the final fallback.
    const result = mapDeletionError({
      code: 'X',
      status: 200,
    });
  ...
  it('classifies empty code + 401 as "uncertain" (status fallback)', () => {
    // Empty code cannot match any code-driven branch. 401 is
    // NOT in `DELETION_RECOVERYABLE_STATUSES`, so the mapper
    // falls through to the final `'uncertain'` bucket.
    const result = mapDeletionError({
      code: '',
      status: 401,
    });
...
// ─── T25.9: branch precedence (regression guards) ────────────────────────────
...
describe('mapDeletionError — branch precedence', () => {
  ...
  it('USER_NOT_FOUND does NOT collapse into "conflict"', () => {
    // Regression guard: a USER_NOT_FOUND on the deletion
    // endpoint is treated as "another tab already deleted";
    // NOT as a retryable conflict.
    const result = mapDeletionError({
      code: USER_NOT_FOUND,
      status: 409,
    });
  ...
  it('AUTH_INVALID_TOKEN does NOT collapse into "uncertain"', () => {
    // A 401 with AUTH_INVALID_TOKEN is deterministic — the
    // status fallback must not be allowed to mask it.
    const result = mapDeletionError({
      code: AUTH_INVALID_TOKEN,
      status: 401,
    });
  ...
  it('AUTH_RESOURCE_CONFLICT wins over retryable status', () => {
    // 503 is retryable by status, but the code-driven branch
    // (AUTH_RESOURCE_CONFLICT) takes precedence.
    const result = mapDeletionError({
      code: AUTH_RESOURCE_CONFLICT,
      status: 503,
    });
...
// ─── T25.11: purity (no side effects) ─────────────────────────────────────────

describe('mapDeletionError — purity', () => {
  it('returns the same classification for the same input', () => {
    // The mapper must be a pure function — repeated calls with
    // the same input return structurally identical results.
  ...
  it('does not mutate the input', () => {
    // Defensive: the input shape is a plain object, but the
    // mapper must not in-place mutate it (e.g. by setting
    // derived fields). A snapshot equality preserves the
    // input's identity.
    const input: DeletionErrorInput = {
      code: AUTH_DELETION_FAILED,
      status: 409,
  ...
  it('does not throw on unknown code', () => {
    // The mapper is exhaustive in its fall-through: even a
    // total nonsense input must produce a non-success
    // classification, not throw.
    expect(() =>
