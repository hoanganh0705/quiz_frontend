/**
 * `BadgeDetail.spec.tsx` — locks the single badge detail surface from
 * TKT-5.5.D3.
 *
 ...
 * - Empty `code` → renders `null`.
 * - Loading → skeleton.
 * - Hidden (private) badge → "Badge hidden" notice.
 * - Deprecated badge → tombstone.
 * - Normal badge → full detail.
 * - Deferred badge → informational note.
 */
...
describe("BadgeDetail", () => {
  ...
  it("renders null when feature flag is placeholder", () => {
    ...
    // The hook is still called before the early return; provide a safe
    // fallback so the destructure in the component succeeds.
  ...
  it("renders the 'not yet available' note when a deferred-badge error is present", () => {
    ...
    // The deferred-badge note is rendered in addition to the description;
    // use `getAllByText` (or a more specific regex) to allow that.
    expect(
