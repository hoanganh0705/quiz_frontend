...
/**
 * `BookmarksDashboardPage` — main dashboard component for bookmark collections.
 *
 * Source epic:   Epic 4.6 — Bookmark collections CRUD + hard-delete confirm.
 * Source ticket: T-4.6-E1.
 *
 * ## What this component owns
 *
 *   - Composes all collection management UI (grid, dialogs, empty states).
 *   - Handles all CRUD operations with optimistic mutations.
 *   - Error handling for 429 (rate limit) and 5xx (server error).
 *   - Loading skeletons during initial hydration.
 */
...
/**
 * `BookmarksDashboardPage` — the main dashboard for managing bookmark collections.
 *
 * Features:
 * - Renders collection grid with skeleton loading state
 * - "New collection" CTA button opens create dialog
 * - Kebab menu on each card: Rename, Change color, Delete
 * - Optimistic mutations with rollback on errors
 * - Toast notifications for rate limit (429) and server (5xx) errors
 */
export default function BookmarksDashboardPage() {
  ...
  const createMutation = useCreateCollection({
    ...
    onError: (code) => {
      // 409 is handled inline by the dialog
      // 429 and 5xx show toasts
  ...
  // ─── Dialog callbacks from CollectionGrid ─────────────────────────────────────
