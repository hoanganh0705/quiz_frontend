/**
 * `bookmarks.service.spec.ts` — locks the bookmarks service contract.
 *
...
describe('bookmarks.service — bulk endpoints per-item shape', () => {
  it('addBookmarksBulk returns the SDK bulk-count envelope', async () => {
    // The SDK's bulk-add endpoint returns
    // `WrappedDto & { data?: { addedCount: number } }` — a count
    // envelope, NOT a per-item result envelope. The F2 ticket body's
    // reference to a `{ ok, results[] }` shape was wrong; the backend
    // responds with a single aggregate count.
    const expected = {
      data: { addedCount: 3 },
    };
