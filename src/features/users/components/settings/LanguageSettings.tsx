...
/**
 * `LanguageSettings` — language + region preferences section.
 *
 ...
 *
 * Reads `locale` from `profile.settings` (passed as prop), renders selects
 * for language, timezone, and date/time formats, and calls
 * `useUpdateMySettings.mutate({ preferences: localeData })` on each change.
 * When `locale.language` changes, `useAppLanguage().setLanguage(newLang)`
 * re-initializes the i18n provider without navigation.
 *
 * ## Diff from the previous version
 *
 * The old component managed state via `useState` and wrote to `localStorage`
 * via an `onUpdate` prop. This version persists to the backend API and
 * re-initializes i18n via `setLanguage()` on language change success.
 */
...
export const LanguageSettings = memo(function LanguageSettings({
  ...
  const handleLocaleChange = useCallback(
    (key: keyof LocaleData, value: string) => {
      ...
      // For language changes, optimistically update the i18n provider
      // before the backend confirms — this makes the UI feel instant.
      if (key === 'language') {
        setLanguage(value);
      }
