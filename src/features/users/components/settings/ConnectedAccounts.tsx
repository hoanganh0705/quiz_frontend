...
export const ConnectedAccounts = memo(function ConnectedAccounts({
  ...
  const [disconnectDialog, setDisconnectDialog] = useState<{
    open: boolean
    provider: keyof typeof settings.connectedAccounts | null
  }>({ open: false, provider: null })

  const handleConnect = (provider: keyof typeof settings.connectedAccounts) => {
  ...
  const handleDisconnect = () => {
    if (disconnectDialog.provider) {
      onUpdate({
        connectedAccounts: {
          ...settings.connectedAccounts,
          [disconnectDialog.provider]: null
        }
      })
  ...
  const connectedCount = Object.values(settings.connectedAccounts).filter(
    (acc) => acc !== null
  ).length
  ...
  return (
    <div className='space-y-6'>
      ...
      <Card className='border-border/40 py-4'>
        ...
        <CardContent className='space-y-3'>
          <AccountProvider
            ...
            account={settings.connectedAccounts.google}
            onConnect={() => handleConnect('google')}
            onDisconnect={() =>
              setDisconnectDialog({ open: true, provider: 'google' })
            }
          />
          <AccountProvider
            name='GitHub'
            ...
            account={settings.connectedAccounts.github}
            onConnect={() => handleConnect('github')}
            onDisconnect={() =>
              setDisconnectDialog({ open: true, provider: 'github' })
            }
          />
          <AccountProvider
            name='Discord'
            ...
            account={settings.connectedAccounts.discord}
            onConnect={() => handleConnect('discord')}
            onDisconnect={() =>
              setDisconnectDialog({ open: true, provider: 'discord' })
            }
          />
          <AccountProvider
            name='X (Twitter)'
            ...
            account={settings.connectedAccounts.twitter}
            onConnect={() => handleConnect('twitter')}
            onDisconnect={() =>
              setDisconnectDialog({ open: true, provider: 'twitter' })
            }
          />
        </CardContent>
      </Card>
      ...
      <Dialog
        ...
        onOpenChange={(open) =>
          setDisconnectDialog({ open, provider: disconnectDialog.provider })
        }
      >
        <DialogContent>
          <DialogHeader>
