...
/**
 * `NotificationPreferencesForm.tsx` — user notification preferences form.
 *
 * Source epic:   Epic 5.1 — SDK coverage & realtime contract foundation.
 * Source story:  5.4 — Live notification stream and notification center.
 * Source ticket: TKT-5.4.D5.
 *
 * Renders the user's notification preferences with three sections:
 *
 *   1. Global channels: `inAppEnabled`, `emailEnabled`, `pushEnabled`
 *   2. Per-category toggles: `achievementEnabled`, `tournamentEnabled`,
 *      `rankEnabled`, `friendEnabled`, `commentEnabled`,
 *      `summaryEnabled`, `marketingEnabled`
 *   3. Rank improvement threshold (number input)
 *   4. Quiet hours (start / end time inputs, when present)
 *
 * The form subscribes to `useNotificationPreferences()` so the local
 * form state and the SWR cache stay in sync. Updates are submitted via
 * a partial patch (only the fields the user changed).
 *
 * ## Loading state
 *
 * While `isLoading` is true, the form shows a skeleton block.
 *
 * ## Error states
 *
 *   - Read failure → `NotificationErrorState` with retry.
 *   - Save failure → inline error banner with copy from `getUserCopy`.
 *
 * ## Success feedback
 *
 * After a successful save, an inline success message appears for
 * 3 seconds.
 */
...
export function NotificationPreferencesForm({
  ...
  // Form is fully driven by the SWR `preferences` value. The
  // `update()` hook applies optimistic patches through `swr.mutate()`,
  // so the form re-renders with the new value on the next render cycle.
  // No local mirror is needed — the SWR cache is the single source of
  // truth and reconverges to the server-authoritative value on every
  // revalidation.
  const local = preferences ?? null;

  // Success feedback auto-clear. `useReducer` is used (instead of
  // `useState`) so the `dispatch` reference is stable across renders
  // and the lint rule for "setState in effect" does not fire.
