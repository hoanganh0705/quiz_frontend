...
/**
 * `features/admin/ranking-admin/components/RecalculateRankingPanel.tsx`
 *
 ...
 *
 * The "Recalculate Rankings" panel: a scope filter input, a
 * `<TypedConfirmDialog>` that requires the recalculate confirm string,
 * a `<RankingCooldownNotice>` when cooldown is active, a
 * `<RankingJobStatusPanel>` for the job lifecycle, and the
 * `<RequestIdBanner>` on failure.
 *
 * The panel preserves its previous state during mutation — a request
 * error does not clear the scope filter value.
 */
...
export interface RecalculateRankingPanelProps {
  /** Optional initial scope filter (period identifier). */
  initialScopeFilter?: string;
}

/**
 * Panel for triggering a ranking recalculation with typed-confirm.
 */
