/**
 * `ReviewReportItem.spec.tsx` — unit tests for the queue's row
 * component.
 *
 * Source epic:   Epic 7.5 — Review moderation queue.
 * Source ticket: TKT-7.5.E1.
 *
 * Coverage contract (TKT-7.5.E1 acceptance criteria):
 *
 *   AC #1 — pending row renders the documented layout
 *           (reporter, reason, status pill, action menu).
 *   AC #2 — resolved row renders `updatedAt` (per A1 evidence:
 *           the SDK does NOT carry `resolvedAt` or `resolvedBy`).
...
describe('TKT-7.5.E1 — ReviewReportItem: action menu wiring', () => {
  it('action selection invokes onAction(action, report) with the typed consumer action', () => {
    ...
    // `review-report-action-{action}-{reportId}`.
