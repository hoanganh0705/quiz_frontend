/**
 * `ReviewReportErrorState.spec.tsx` — unit tests for the queue's
 * error state.
 *
 ...
 * Coverage contract:
 *
 *   - Renders the documented copy per error-code bucket:
 *     - `GLOBAL_FORBIDDEN` / `PERMISSION_DENIED` → "Permission denied".
 *     - `REVIEW_NOT_FOUND` / `GLOBAL_NOT_FOUND` → "Reports not available".
 *     - `GLOBAL_RATE_LIMITED` → "Too many requests".
 *     - `GLOBAL_NETWORK_ERROR` / `GLOBAL_TIMEOUT` → "Connection lost".
 *     - `GLOBAL_INTERNAL_ERROR` → "Server error".
 *     - any other / missing code → "Could not load reports".
 *   - Surfaces the typed `error.code` verbatim so admins can
 *     correlate the failure with backend tooling.
 *   - Clicking the retry button invokes `onRetry` exactly once.
 *   - The `RequestIdBanner` renders when `error.requestId` is set.
 *
...
describe('TKT-7.5.D3 — ReviewReportErrorState', () => {
  ...
  it('renders the request id banner when requestId is present', () => {
    ...
    // RequestIdBanner renders the requestId verbatim when present.
