/**
 * `useHideComment` / `useRestoreComment` unit tests.
 *
 * Source epic:   Epic 7.6 — Comment Moderation (Hide, Restore, and Report Queue).
 * Source ticket: TKT-7.6.C3.
 *
 * Coverage map (TKT-7.6.C3 acceptance criteria):
 *
 *   AC #1 — `hide()` calls the SDK `hideComment` with the comment id
 *   AC #2 — `restore()` calls the SDK `restoreComment` with the comment id
 *   AC #3 — success emits `started` + `success` breadcrumbs with the
 *           documented `breadcrumbAction` strings
 *   AC #4 — success invalidates the comment-reports queue, the
 *           `comments:byId:${commentId}` key, and every `comments:*`
 *           namespace key
 *   AC #5 — `COMMENT_ALREADY_HIDDEN` outcome is exposed through
 *           `lastOutcome.kind === 'already-hidden'` and the hook
 *           revalidates the cache
 *   AC #6 — `COMMENT_NOT_HIDDEN` outcome is exposed through
 *           `lastOutcome.kind === 'not-hidden'` and the hook
 *           revalidates the cache
 *   AC #7 — `COMMENT_NOT_FOUND` outcome is exposed through
 *           `lastOutcome.kind === 'not-found'`
 *   AC #8 — `GLOBAL_FORBIDDEN` outcome is exposed through
 *           `lastOutcome.kind === 'forbidden'`
 *   AC #9 — failure emits a `failure` breadcrumb with `code`,
 *           `requestId`, `correlationId`
 *   AC #10 — non-typed errors (non-`ApiError`) are wrapped into an
 *            `ApiError` with `code: 'UnknownError'`
 *   AC #11 — `reset()` clears state
 *   AC #12 — concurrent calls dedupe to the in-flight Promise
