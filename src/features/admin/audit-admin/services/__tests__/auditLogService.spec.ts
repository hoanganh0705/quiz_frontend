/**
 * `auditLogService.spec.ts` — Audit log service contract tests.
 *
 * Source epic:   Epic 7.11 — Admin Audit Log Surface and Backend Capability Verification.
 * Source ticket: TKT-7.11.I1.
 *
 * Tests the audit log service layer:
 *   - listAuditLog: success case
 *   - listAuditLog: error cases (PERMISSION_DENIED, NOT_EXPOSED)
 *   - getAuditLogEntry: success case
 *   - getAuditLogEntry: error cases
 *   - filter validation (delegated to utils tests; here we verify the
 *     service passes filters through without alteration)
...
describe('auditLogService', () => {
  ...
  // ─── listAuditLog: filter passes through ──────────────────────────────
  ...
  // ─── listAuditLog: pagination edge cases ──────────────────────────────
  ...
  // ─── getAuditLogEntry: error contracts ────────────────────────────────

  describe('getAuditLogEntry error contracts', () => {
    it('throws when entryId is empty', async () => {
      // The mock implementation doesn't actually validate, but document
      // the expected behaviour for the production service.
