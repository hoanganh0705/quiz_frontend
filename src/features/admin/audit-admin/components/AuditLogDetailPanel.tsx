 ...
 * Source ticket: TKT-7.11.D5.
 *
 * ## What this component owns
 *
 * Slide-over or modal panel for viewing full audit entry details.
 * Displays all entry fields prominently:
 ...
 *
 * The `payload` field is redacted server-side per
 * `docs/AUDIT_ENDPOINT_CONTRACT.md`. This component shows only
 * the `payload` field is protected — never rendering raw payload data.
 */
...
export interface AuditLogDetailPanelProps {
  /** The entry ID to display, or null when panel is closed. */
  entryId: string | null;
  /** Optional: pre-loaded entry to display while fetching full details. */
  initialEntry?: AuditLogEntryDto | null;
  /** Invoked when the panel should close. */
  onClose: () => void;
}
...
export function AuditLogDetailPanel({
  ...
  // Prefer the freshly fetched entry, fall back to initialEntry
