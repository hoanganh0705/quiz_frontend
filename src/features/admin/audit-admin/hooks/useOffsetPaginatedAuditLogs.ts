...
/**
 * `useOffsetPaginatedAuditLogs.ts`
 *
 ...
 * ## Naming history
 *
 * Previously exported as `useOffsetPaginated` from this folder, which
 * collided with the Phase-6 `@/lib/api/use-offset-paginated.ts`
 * facade (TKT-7.5 cleanup plan, Phase 5 / P1-2). The audit-log
 * variant is the *page-navigation* hook (offset state + nav helpers)
 * while the Phase-6 hook is a *fetch* facade. Renamed to
 * `useOffsetPaginatedAuditLogs` so callers must import the variant
 * they actually want.
 *
 * ## SSR-safety
 *
 ...
 *     useOffsetPaginatedAuditLogs({ total: 150, initialLimit: 20 });
 */
...
/**
 * @deprecated Use `useOffsetPaginatedAuditLogs` instead. The original
 * name `useOffsetPaginated` collided with the Phase-6
 * `@/lib/api/use-offset-paginated` facade (Phase 5 / P1-2 of the
 * cleanup plan). Re-exported under both names for one release to
 * give callers time to migrate.
 */
