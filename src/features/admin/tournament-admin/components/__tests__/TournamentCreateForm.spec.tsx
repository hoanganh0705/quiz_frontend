/**
 * `TournamentCreateForm` unit tests.
 *
 * Source epic:   Epic 7.7 — Tournament Admin: Create, Update, Delete.
 * Source ticket: TKT-7.7.D2.
 *
 * Coverage map (TKT-7.7.D2 acceptance criteria):
 *
 *   AC #1  — every documented field renders with the right input
 *           primitive.
 *   AC #2  — client-side validation prevents submit when `startAt`
 *           is in the past, `endAt <= startAt`, or `maxParticipants`
 *           is out of bounds.
 *   AC #3  — submit invokes `useCreateTournament().create(input)`.
 *   AC #4  — success closes the form by calling `onSuccess(tournament)`.
 *   AC #5  — `TOURNAMENT_VALIDATION` → inline form error banner.
 *   AC #6  — `TOURNAMENT_SLUG_CONFLICT` → inline form error banner.
 *   AC #7  — `ADMIN_FORBIDDEN` → `RequestIdBanner`.
 *   AC #8  — `pnpm type-check` exits 0 (handled outside the suite).
 *
 * Notes on the test strategy:
 *
 *   - The `useCreateTournament` hook is mocked at the module level.
 *     The mock owns the form's `isPending` and `error` shape so the
 *     test can drive the component through every error branch.
 *   - `AuditActionShell` is **not** mocked. It is the audit primitive
 *     the form wraps the mutation in. Driving the real shell lets us
 *     cover the started → success breadcrumb path end-to-end.
 *   - `RequestIdBanner` is rendered by the form only when the
 *     `useCreateTournament` mock returns an `error` with a requestId.
 */
...
// Pick start/end times that are safely in the future relative to
// the wall-clock at the time the test runs.
...
function renderForm(onSuccess = vi.fn(), onCancel = vi.fn()) {
  // Pin the "now" anchor so the test does not race against the
  // wall-clock between page navigation and submit.
  return render(
...
describe('TKT-7.7.D2 — TournamentCreateForm: client-side validation', () => {
  ...
  it('AC #2: surfaces inline errors when the form is submitted invalid', () => {
    ...
    // The native form submit fires when the user presses Enter
    // inside a form input. That triggers the form's `onSubmit`
    // handler which sets `touched` to true, which in turn renders
