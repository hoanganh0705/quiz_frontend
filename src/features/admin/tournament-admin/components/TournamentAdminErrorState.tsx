/**
 * `TournamentAdminErrorState` — error state for the tournament admin list.
 *
 * Source epic:   Epic 7.7 — Tournament Admin: Create, Update, Delete.
 * Source ticket: TKT-7.7.D5 (AC #3).
 *
 * Renders the typed error code and a retry CTA. The component does not
 * make any server calls — the retry callback is owned by the caller.
 */
...
export interface TournamentAdminErrorStateProps {
  /** The typed API error from the list hook. */
  error: ApiError;
  /** Invoked when the admin clicks "Try again". */
  onRetry: () => void;
}
