/**
 * `__tests__/useTagSlugAvailability.spec.tsx`
 *
...
describe('useTagSlugAvailability', () => {
  ...
  it('returns invalid for a slug that fails the regex', () => {
    // Uppercase slugs are rejected by the regex (lowercase-only pattern).
  ...
  it('excludes the specified tag id (self-editing)', () => {
    // tag-1 is excluded, so javascript should appear available.
  ...
  it('is case-insensitive', () => {
    // 'javascript' is in ITEMS as 'javascript'. Matching is case-insensitive.
    const { result } = renderHook(() => useTagSlugAvailability('javascript'));
    expect(result.current.status).toBe('taken');
  });
});
