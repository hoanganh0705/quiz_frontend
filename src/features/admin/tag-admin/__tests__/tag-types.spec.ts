/**
 * `features/admin/tag-admin/__tests__/tag-types.spec.ts`
 *
...
describe('discriminated union — TagListItem', () => {
  ...
  it('rejects deletedAt as a string', () => {
    ...
    // We verify the type shape is correct by demonstrating the valid case.
    const active: TagListItem = {
      tagId: '1',
      name: 'Active Tag',
      slug: 'active-tag',
...
describe('discriminated union — DeletedTagListItem', () => {
  ...
  it('rejects deletedAt as null', () => {
    // TypeScript would reject this at compile time.
    // Verify the type shape is correct by demonstrating the valid case.
    const deleted: DeletedTagListItem = {
      tagId: '2',
      name: 'Deleted Tag',
      slug: 'deleted-tag',
