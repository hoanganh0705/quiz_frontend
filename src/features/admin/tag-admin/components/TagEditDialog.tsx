...
/**
 * `features/admin/tag-admin/components/TagEditDialog.tsx`
 *
 * Source epic:   Epic 7.3.
 * Source ticket: TKT-7.3.E2.
 *
 * ## Purpose
 *
 * Dialog for editing an existing tag. Wraps `useUpdateTag` and
 * `TagFormFields` (edit mode). On `TAG_NOT_FOUND` or `TAG_SLUG_CONFLICT`,
 * surfaces the appropriate notice. On `TAG_SLUG_CONFLICT`, includes
 * the rename path via `SlugConflictNotice`.
 */
...
export interface TagEditDialogProps {
  ...
  /** The tag to edit. `null` means the dialog is closed / no tag selected. */
  tag: TagDto | null;
  /** Called with the updated tag on success. */
...
export function TagEditDialog({
  ...
  // Sync form state when the selected tag changes while the dialog stays open.
  useEffect(() => {
