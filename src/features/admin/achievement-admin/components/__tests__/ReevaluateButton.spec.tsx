/**
 * `ReevaluateButton` unit tests.
 *
 * Source epic:   Epic 7.8 — Achievement Admin: Re-evaluate per User and Revoke Badge.
 * Source ticket: TKT-7.8.D1.
 *
 * Coverage map (TKT-7.8.D1 acceptance criteria):
 *
 *   AC #1  — idle → primary button "Re-evaluate achievements"; click invokes reevaluate().
 *   AC #2  — running → secondary button "Re-evaluation running…" with spinner; click is no-op.
 *   AC #3  — completed → primary button "Re-evaluate again".
 *   AC #4  — failed → primary button "Retry re-evaluation".
 *   AC #5  — REVAL_RUNNING notice renders when typed code is returned.
 *   AC #6  — permission denied → PermissionDeniedNoticeInline renders.
 *   AC #7  — non-REVAL_RUNNING error surfaces RequestIdBannerInline.
 */
