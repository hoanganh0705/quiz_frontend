/**
 * `tag-admin.service.spec.ts` — Locks the tag admin service contract
 * (TKT-7.1.E1).
 *
 * Verifies:
 *   - Each service function calls the corresponding SDK function.
 *   - Each function unwraps the SDK envelope and returns the
 *     canonical `TagResponseDto`.
 *   - SDK errors propagate as `ApiError` (the service does not
 *     swallow them).
 *   - `restoreTag` documents `TAG_SLUG_CONFLICT` (the spec file
 *     confirms the JSDoc is present by reading the source).
 */
