...
/**
 * `features/admin/category-admin/components/CategoryAdminList.tsx`
 *
 * Source epic:   Epic 7.4 — Category admin CRUD + restore.
 * Source ticket: TKT-7.4.F1.
 *
 * Renders the active / soft-deleted tab interface backed by
 * `useCategoryAdminList`. Each tab shows the appropriate rows with
 * action buttons that open the corresponding dialogs. Tab state is
 * stored in the URL search param so deep-links and back/forward
 * navigation work as expected.
 *
 * Action buttons are gated on the documented `usePermission` checks:
 *   - update + delete gates on the active tab
 *   - restore gate on the soft-deleted tab
 */
...
export interface CategoryAdminListProps {
  /** Called when the user clicks the "Add Category" CTA in the empty active state. */
  onAddCategory?: () => void;
}
