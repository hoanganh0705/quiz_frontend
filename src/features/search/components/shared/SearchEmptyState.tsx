...
/**
 * `SearchEmptyState.tsx` — empty-state block for search surfaces.
 *
 * Source epic:   Epic 5.1 — SDK coverage & realtime contract foundation.
 * Source story:  5.6 — Search and Approved Read-Only Social Discovery Integration.
 * Source ticket: TKT-5.6.C1.
 *
 * Provides three variants driven by copy from `getUserCopy`:
 *   - `'no-query'`       — input has not been filled yet
 *   - `'no-results'`     — query returned zero results
 *   - `'no-results-in-group'` — group filtering yielded nothing
 *
...
interface SearchEmptyStateProps {
  /** Controls which icon, title, and body copy is shown. */
  variant?: SearchEmptyStateVariant;
  /** Optional label for the active group filter when variant is 'no-results-in-group'. */
  groupLabel?: string;
  className?: string;
}
...
export function SearchEmptyState({
  ...
  // Build description with optional group label for no-results-in-group
  const description =
