/**
 * `SearchResultGroup.spec.tsx` — locks the search result group component.
 *
 ...
 *
 * - Renders items matching the group's `kind`.
 * - Filters private items for anonymous users.
 * - Renders authenticated-only hint for private items.
 * - Renders 'no-results-in-group' empty state when all items are filtered.
 * - Injects footer (for load-more button) when provided.
 * - No unstable social IDs in rendered links.
 */
...
describe("SearchResultGroup", () => {
  ...
  describe("privacy filtering (TKT-5.6.G2 AC #2)", () => {
    ...
    it("shows authenticated-only hint when authenticated items are filtered for anonymous viewers (TKT-5.6.G2 AC #2)", () => {
      ...
      // Authenticated-only items filtered → "Sign in" prompt shown
    ...
    it("renders 'no-results-in-group' empty state when all items are filtered", () => {
      ...
      // No auth prompt because items are private, not authenticated —
      // component renders the generic SearchEmptyState
