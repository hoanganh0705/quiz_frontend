/**
 * `phase5-5-1.integration.spec.tsx` — cross-feature integration smoke check
 * for Story 5.1 (Epic 5.1, TKT-5.1.H1).
 *
 * Exercises the full Story 5.1 stack end-to-end against mocked Socket.IO
 * and mocked SDK responses, asserting:
 *
 *   1. **useSocket → connected**  — hook enters `connected` state when the
 *      mock socket fires `connect`.
 *
 *   2. **useRealtimeQuery invalidation** — when the mock socket fires a
 *      `notification:sent` event, SWR is invalidated for the matching key.
 *
 *   3. **WS error → authRequired** — when the mock socket fires a
 *      `connect_error` that decodes to `authRequired: true`, the hook
 *      transitions to `auth_required` state.
 *
 *   4. **Service wrappers normalize** — every Phase 5 service unwraps the
 *      `data` envelope and propagates `ApiError.code`.
 *
 * ## Mocking strategy
 *
 * Mirrors `phase4-4-1.integration.spec.tsx`:
 *
 *   - `vi.mock` replaces each service module at the module level.
 *     The mock stubs the exported function so it returns the **unwrapped**
 *     data directly — the stub bypasses the internal SDK call, so no SDK
 *     mock is needed.
 *   - Each test case does a **dynamic** `await import(...)` to get the
 *     mocked module reference, then configures it via
 *     `.mockResolvedValue(...)` / `.mockRejectedValue(...)`.
 *   - `beforeEach(vi.clearAllMocks())` resets call history between tests.
 *
 * @see useSocket (TKT-5.1.E1)
 * @see useRealtimeEvent (TKT-5.1.E2)
 * @see useRealtimeQuery (TKT-5.1.E3)
 * @see tournaments.service (TKT-5.1.F1)
 * @see instances.service (TKT-5.1.F2)
 * @see notifications.service (TKT-5.1.F3)
 * @see rankings.service (TKT-5.1.F4)
 * @see achievements.service (TKT-5.1.F5)
 * @see search.service (TKT-5.1.F6)
 */
...
// ─── Mock ws-error at module level ──────────────────────────────────────────
// Declared before imports so Vitest hoists it. All connect_error payloads
// use the AUTH_SESSION_EXPIRED shape.
...
//
// Each vi.mock stubs the exported function to return the unwrapped data directly.
// This bypasses the internal SDK call so no SDK mock is needed. The stub
// mirrors Phase 4's pattern: the mock replaces the module, each test imports
// the stub via dynamic import, then configures it with mockResolvedValue /
// mockRejectedValue.
...
describe("Story 5.1 — useSocket enters connected state", () => {
  ...
  it("transitions to connected when mock socket fires connect", async () => {
    ...
    // Initial state is `connecting` (useEffect runs synchronously on mount)
...
// ─── Test 2: useRealtimeQuery — SWR invalidation on event ───────────────────
...
describe("Story 5.1 — useRealtimeQuery invalidates SWR on notification:sent", () => {
  ...
  it("invalidates the SWR key when notification:sent fires", async () => {
    ...
    // Wait for SWR to resolve — fetcher returns a Promise that resolves
    await waitFor(
      () => {
    ...
    // Simulate notification:sent event — should trigger SWR mutate
    mockSocket._emit("connect");
    ...
    // The invalidation triggers revalidation; the fetcher is called again
    await waitFor(
      () => {
  ...
  it("does not subscribe when swrKey is null", () => {
    ...
    // Fetcher must not be called with a null key
    expect(fetcher).not.toHaveBeenCalled();
    // Socket must not register listeners (no on() call)
    expect(mockSocket.on).not.toHaveBeenCalled();
  });
});

// ─── Test 3: useSocket — authRequired on connect_error ───────────────────────
...
describe("Story 5.1 — useSocket transitions to auth_required on WsError", () => {
  ...
  it("enters auth_required when connect_error has authRequired: true", async () => {
    ...
    // Simulate auth error — decodeWsError mock returns authRequired: true
...
// ─── Test 4: Service wrappers normalize data envelope + propagate ApiError ────
...
describe("Story 5.1 — Phase 5 service wrappers normalize responses", () => {
  ...
  it("notificationsService.listNotifications returns unwrapped data", async () => {
    ...
    // The stub (vi.mock) bypasses the SDK call and returns this directly.
    // The value represents the **unwrapped** data the service returns to callers.
  ...
  it("achievementsService.listBadges normalizes bare array input", async () => {
    ...
    // Backend may return a bare array (without envelope).
    // The service's normalizeBadgeArray handles this — the stub returns
    // the unwrapped normalized output (an array).
