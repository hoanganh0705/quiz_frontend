/**
 * `useQuizFiltersStore` — unit spec.
 *
 * Source epic: Epic 3.5 — Global quizzes list + filters.
 * Source ticket: TKT-3.5.C4.
 *
 * Locks the store's typed API:
 *
 *   (a) initial state is `{}`.
 *   (b) `setFilter('sort', 'popular')` updates the field.
 *   (c) `resetFilters()` clears the state.
 *   (d) `getUrlSearchParams()` returns the serialised state.
 *   (e) `setFromUrlSearchParams(new URLSearchParams(...))` updates
 *       the state.
 *   (f) round-trip identity: `setFromUrlSearchParams(getUrlSearchParams())`
 *       produces the same state.
 *   (g) `setFilter(key, undefined)` removes the field from the state.
 */
...
describe('useQuizFiltersStore — round-trip identity', () => {
  it('setFromUrlSearchParams(getUrlSearchParams()) is the identity', () => {
    // Set a non-trivial state via the public API.
    setFilter('categoryId', 'cat-123')
    setFilter('sort', 'popular')
    setFilter('difficulty', 'easy')
    setFilter('tagSlugs', ['a', 'b'])
