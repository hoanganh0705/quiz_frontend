/**
 * Quiz Types — aligned with backend DTOs.
 *
 * Backend returns:
 * - QuizResponseDto (quiz metadata)
 * - QuizVersionResponseDto (version with optional questions)
 * - QuizQuestionResponseDto (question)
 * - QuizAnswerOptionResponseDto (answer option)
 *
 * Frontend needs:
 * - QuizListItem: QuizResponseDto + embedded version summary (for listings)
 * - QuizDetail: QuizResponseDto + full version with questions (for taking quiz)
 *
 * Security: isCorrect is NEVER returned in quiz questions (prevents cheating)
 */

// ─── Backend-aligned types (matching DTOs exactly) ───────────────────────────────

export type QuizDifficulty = 'easy' | 'medium' | 'hard'
export type QuizVersionStatus = 'draft' | 'published' | 'archived'

// Quiz listing item — summary without questions
...
// Embedded in quiz listing (no questions)
...
// Full version with questions (for quiz detail / taking)
...
// ─── Legacy types (for backward compat with existing UI) ─────────────────────────

/** @deprecated Use QuizDifficulty instead (backend uses lowercase) */
export type QuizDifficultyLegacy = 'Easy' | 'Medium' | 'Hard'

/** @deprecated — quiz listing item with frontend-compatible field names */
...
/**
 * Map backend QuizResponseDto to frontend QuizListItem
 * Used for quiz listings/catalogs
 */
...
/**
 * Map backend QuizVersionDetailDto to frontend QuizDetail
 * Used when taking a quiz
 */
