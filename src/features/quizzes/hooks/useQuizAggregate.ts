...
/**
 * `useQuizAggregate` — Phase 4 (S-24) hook for the
 * `GET /quizzes/:id/aggregate` composite endpoint.
 *
 * The quiz detail page used to fan out 5+ sequential calls
 * (quiz, stats, stats history, preview questions, etc.). The
 * aggregate endpoint collapses the fan-out into a single
 * parallelised round-trip.
 *
 * The hook returns the bundle's four projections:
