/**
 * `useMyQuizzes.spec.tsx` — unit tests for the `useMyQuizzes` hook.
 *
 * Source epic:   Epic 4.4 — Authored quizzes list + analytics.
 * Source ticket: TKT-4.4.E3.
 *
 * Five cases per the ticket's testing checklist:
 *
 *   (1) First render returns `isLoading: true` before SWR resolves.
 *   (2) Each item has a synthesised `id` matching its `quizId`.
 *   (3) 404 → empty items array (not thrown).
 *   (4) 5xx propagates as `ApiError`.
 *   (5) SWR key includes `['quizzes', 'me', 'all']`.
 *
...
describe("useMyQuizzes — shape on first render", () => {
  ...
  it("returns all documented fields after resolving", async () => {
    ...
    // Verify the full shape at the type level by accessing each field.
...
describe("useMyQuizzes — id synthesised from quizId", () => {
  it("each item carries a string id equal to its quizId", () => {
    // The synthesisation happens in the fetcher. We verify the fixture
    // structure here; the actual `id` field is added by the fetcher.
...
describe("useMyQuizzes — 404 → empty items", () => {
  it("returns an empty items array on 404", async () => {
    ...
    // 404 is caught in the fetcher and normalised to empty items.
...
describe("useMyQuizzes — SWR key", () => {
  it("SWR key includes the 'all' discriminator", () => {
    // The key factory is tested directly.
