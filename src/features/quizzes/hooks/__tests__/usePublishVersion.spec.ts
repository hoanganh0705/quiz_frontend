/**
 * `usePublishVersion.spec.ts` — unit tests for publish version hook.
 *
 * Source epic:   Epic 4.11 — Quiz publish flow + edit-published-as-new-draft.
 * Source ticket: T-4.11.6.
 *
 * Tests the usePublishVersion hook for:
 * - Success path
 * - Error handling (QUIZ_INSUFFICIENT_QUESTIONS, QUIZ_VERSION_IMMUTABLE, 429, 5xx)
...
/**
 * Build an ApiError that mimics the RFC 7807 wire shape.
 * The code lives in extensions.code per the ApiError class implementation.
 */
function makeApiError(
  ...
  return new ApiError({
    ...
    name: 'AxiosError',
    message,
    code: undefined, // Not used by ApiError
    config: undefined,
    response: {
      status,
      statusText: message,
...
describe('usePublishVersion service contract', () => {
  ...
  it('throws on unexpected response shape (no data field)', async () => {
    mockPublishQuizVersion.mockResolvedValueOnce({}); // No data field
