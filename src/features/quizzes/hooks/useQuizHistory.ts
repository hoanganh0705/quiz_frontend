/**
 * `useQuizHistory` — paginated quiz history with filter state.
 *
 * Phase 5 (S-29): wraps the new `GET /users/me/quiz-history` endpoint
 * and exposes the friendly `QuizHistoryEntry` shape the editor already
 * renders. Cursor pagination is plumbed through; the returned `loadMore`
 * callback fetches the next page when there is one.
 *
 * Source epic: Epic 4.10 — Question editor.
 * Source ticket: T-5.29.1.
 */
'use client';
...
/**
 * Map the backend's `QuizHistoryEntryDto` into the editor's existing
 * `QuizHistoryEntry` shape. The backend's friendly entry shape is a
 * strict superset of what the timeline needs, so the mapping is direct.
 */
  ...
  return {
    ...
    status:
      row.status === 'in_progress'
        ? 'failed' // timeline does not render in_progress; collapse to failed placeholder
        : row.status,
