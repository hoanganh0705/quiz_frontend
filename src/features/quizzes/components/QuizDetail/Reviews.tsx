...
import { listReviews } from "@/features/reviews";
...
export default function Reviews({ quizId }: ReviewsProps) {
  ...
  useEffect(() => {
    async function fetchReviews() {
      try {
        const data = await listReviews(quizId, { limit: 20 });
        setReviews(((data as unknown as { data?: ReviewResponseDto[] }).data ?? []) as ReviewResponseDto[]);
      } catch {
        ...
        <div className="mx-auto">
          ...
          <div className="space-y-4">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="border border-border rounded-lg p-6 bg-background animate-pulse"
              >
