/**
 * `MyQuizzesTableRow.spec.tsx` — RTL unit tests for `<MyQuizzesTableRow />`.
 *
 * Source epic:   Epic 4.4 — Authored quizzes list + analytics.
 * Source ticket: TKT-4.4.E7.
 *
 * Five cases per the ticket's testing checklist:
 *
 *   (1) Renders all columns for a non-deleted quiz.
 *   (2) Long title is clamped (has `line-clamp-2` class) and full title
 *       is in `aria-label`.
 *   (3) Soft-deleted quiz renders "Deleted" badge.
 *   (4) Soft-deleted quiz has no Edit button.
 *   (5) Non-deleted quiz has an Edit button linking to `/my-quizzes/[id]`.
 */
