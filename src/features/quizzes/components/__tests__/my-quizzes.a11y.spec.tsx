/**
 * `my-quizzes.a11y.spec.tsx` — axe-core accessibility audit for the
 * author's quiz dashboard components.
 *
 * Source epic:   Epic 4.4 — Authored quizzes list + analytics.
 * Source ticket: TKT-4.4.F2.
 *
 * Verifies WCAG 2.1 AA compliance for three required states:
 *
 *   - **Live table row** — the table row with columns, status badges,
 *     and Edit links.
 *   - **Deleted table row** — soft-deleted row with "Deleted" badge.
 *   - **Analytics tab** — the stat cards.
 *   - **Empty state** — the `MyQuizzesTableEmpty` variant.
 *
 * Color contrast is verified manually (jsdom does not implement
 * HTMLCanvasElement; the project-wide convention per
 * `axe-quiz-card.spec.tsx`).
 */
...
async function runAxe(container: HTMLElement): Promise<axe.AxeResults> {
  return await new Promise((resolve, reject) => {
    axe.run(
      container,
      {
        // Only audit the rendered subtree.
...
describe("MyQuizzesTableRow — a11y (deleted row)", () => {
  it("no critical or serious axe violations on a deleted row", async () => {
    ...
    // Deleted badge must be present and announced.
