...
/**
 * `HistoryStatsDashboard` — phase 5 (S-29) wired version.
 *
 * The legacy tree under `features/quizzes/components/QuizHistory/*` is
 * a pre-existing component set that was never mounted by any page.
 * Phase 5 wires it to the real backend via `useQuizHistoryStats` so
 * the rich stat visuals (KPI cards, weekly activity chart, category
 * pie chart) finally have a real data source.
 *
 * If the live `QuizHistoryPage` from `features/attempts` is mounted
 * instead, this dashboard can still be slotted in as a sibling
 * component below the filter bar.
 */
...
interface HistoryStatsDashboardProps {
  /**
   * Optional override. When omitted the dashboard fetches stats from
   * the backend itself via `useQuizHistoryStats`. Pass-through for
   * storybook / testing.
   */
...
export const HistoryStatsDashboard = memo(function HistoryStatsDashboard({
  ...
  // Best-effort weekly activity stub — the backend `/stats` endpoint
  // doesn't expose bucketed activity yet, so the chart renders an
  // empty skeleton rather than fabricating numbers. Will be wired
  // when a follow-up endpoint exposes bucketed activity.
...
// Re-export for clients that still need the legacy type
