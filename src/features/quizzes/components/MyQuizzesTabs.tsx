/**
 * `MyQuizzesTabs` — tab switcher for the author's dashboard.
 *
 * Source epic:   Epic 4.4 — Authored quizzes list + analytics.
 * Source ticket: TKT-4.4.D3.
 *
 * Renders 4 tabs (All / Drafts / Published / Analytics) using the existing
 * `<Tabs>` primitive. Reads `activeTab` from `MyQuizzesTabStore` and emits
 * a Sentry breadcrumb on every tab switch.
 */
...
interface MyQuizzesTabsProps {
  /** Content rendered for each tab panel. Receives the tab value. */
  children: (tab: "all" | "drafts" | "published") => React.ReactNode;
  /** Analytics tab content. Rendered when analytics tab is active. */
...
/** Emit a Sentry breadcrumb for tab-switching. Lazy-loaded to avoid circular deps. */
...
/**
 * Tab switcher for the author's dashboard.
 */
