/**
 * `useDeleteAnswer.spec.tsx` — locks the answer-withdrawal mutation
 * hook.
 *
 ...
 * Coverage contract:
 *
 *   - Service receives the verified `attemptId` and `questionId`.
 *   - Success removes the lock and revalidates the answers cache.
 *   - Success emits one cross-tab `attempts/changed { kind: 'withdraw' }`.
 *   - 404 `ATTEMPT_ANSWER_NOT_FOUND` produces `already_missing` (silent)
 *     and clears the local lock without surfacing an error.
 *   - 404 `ATTEMPT_NOT_FOUND` produces `not_found`.
 *   - 409 `ATTEMPT_NOT_ACTIVE` produces `not_active`.
 *   - 403 produces `forbidden`.
