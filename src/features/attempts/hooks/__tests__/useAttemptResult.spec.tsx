/**
 * `useAttemptResult.spec.tsx` — locks the attempt-result read hook.
 *
 ...
 * Coverage:
 *
 *   - Auth-gating: no fetch fires while auth is unresolved.
 *   - Auth bootstrap loading: `result` stays `null` and `isLoading`
 *     stays `false` until the bootstrap settles.
 *   - 200 returns the canonical `AttemptResultDto` projection.
 *   - 404 returns `result: null` with no error (the documented
 *     "no result yet" projection).
 *   - 401 / 403 / 429 / 5xx remain typed errors.
 *   - Switching attempt ids changes the cache key.
 *   - The hook is read-only and never calls `completeAttempt`.
 */
...
describe('useAttemptResult — error and no-result', () => {
  ...
  it('403 ATTEMPT_FORBIDDEN surfaces a typed ApiError (not null)', async () => {
    ...
    // Misclassification guard: 403 must NOT collapse to `null`.
    expect(result.current.result).toBeNull();
