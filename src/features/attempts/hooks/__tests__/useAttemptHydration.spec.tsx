/**
 * `useAttemptHydration.spec.tsx` — locks the Story 4.14 canonical
 * attempt hydration hook.
 *
 ...
 * Coverage contract:
 *
 *   - Authenticated + valid attempt id triggers both reads.
 *   - The submitted-answers projection maps every server-confirmed
 *     answer exactly once.
 *   - The hydration projection contains no `isCorrect` (player-DTO
 *     invariant).
 *   - 403 and 404 remain distinct errors.
 *   - `null` attemptId disables the fetch.
 *   - Bootstrap-loading and unauthenticated states fire no request.
 *   - Refresh revalidates both cache keys.
 *   - The hook does NOT call attempt analytics or attempt review.
 */
...
describe('useAttemptHydration — happy path', () => {
  ...
  it('hydration projection contains no isCorrect field', async () => {
    ...
    // Player-DTO invariant: no correctness metadata on the runner
    // surface. The fixture deliberately omits `isCorrect`; a future
    // wire-shape change that tried to add it would surface here.
