/**
 * `useAbandonAttempt.spec.tsx` — locks the typed-confirmation abandon
 * mutation hook.
 *
 ...
 * Coverage contract:
 *
 *   - Rendering the hook fires no mutation.
 *   - `confirm()` fires the service exactly once per logical intent.
 *   - Success hydrates the runner store to terminal `abandoned`.
 *   - Success revalidates active + detail caches.
 *   - Success emits one cross-tab `attempts/changed { kind: 'abandon' }`.
 *   - 409 `ATTEMPT_NOT_ACTIVE` produces `not_active` and revalidates
 *     the attempt detail so the runner can converge to the terminal
 *     status.
 *   - 403 produces `forbidden`; 404 `ATTEMPT_NOT_FOUND` produces
 *     `not_found`.
 *   - 429 / 5xx surface as `retryable` with the typed error.
 *   - Cooldown blocks rapid duplicate clicks.
 *   - The complete mutation is never invoked.
 */
