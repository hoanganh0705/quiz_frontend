/**
 * `QuizHistoryRow.spec.tsx` — locks the attempt history row.
 *
 ...
 * Coverage contract:
 *
 *   - Renders quiz title, completion timestamp, score, and a status
 *     badge for `completed` / `abandoned` / `started`.
 *   - Renders a stable link to `/quiz-history/[attemptId]`.
 *   - Renders nothing when the attemptId is missing.
 *   - Accessible queries find the row and its status badge.
...
describe('QuizHistoryRow — missing identity', () => {
  ...
  it('renders nothing when the row has no id or attemptId', () => {
    // @ts-expect-error — testing a deliberately incomplete row.
