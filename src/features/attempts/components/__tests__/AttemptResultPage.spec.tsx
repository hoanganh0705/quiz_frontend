/**
 * `AttemptResultPage.spec.tsx` — locks the result-page composition.
 *
 ...
 * Coverage contract:
 *
 *   - Renders the loading skeleton while the result query loads.
 *   - Renders the empty-result fallback when the result resolves to `null`.
 *   - Renders the score hero, breakdown, feedback, and review CTA
 *     in the approved order when the result is present.
 *   - Renders the swap-on view for `ATTEMPT_NOT_ACTIVE` without a
 *     toast.
 *   - Renders the inline validation banner for
 *     `ATTEMPT_VALIDATION_FAILED` and keeps the runner mounted.
 *   - Renders the redirect + toast for `ATTEMPT_NOT_FOUND` /
 *     `ATTEMPT_FORBIDDEN`.
 *   - Renders a generic retry banner for 5xx / 401 / 429 errors.
 *   - The page does not import author or pre-completion review DTOs.
 *   - No completion polling is invoked.
 */
