/**
 * `QuizHistoryList.spec.tsx` — locks the paginated history list composition.
 *
 ...
 * Coverage contract:
 *
 *   - Skeleton renders during the first fetch.
 *   - Rows render from the hook's items.
 *   - Load-more appears when `hasMore`.
 *   - Empty-state CTA renders when items are empty.
 *   - Retry affordance renders on 5xx error.
...
describe('QuizHistoryList — loading skeleton', () => {
  it('renders 10 skeleton rows while loading', () => {
    ...
    // The skeleton is a single container with multiple Skeleton children.
    expect(screen.getByTestId('quiz-history-list-skeleton')).toBeInTheDocument();
    // Verify 10 skeleton rows exist inside.
