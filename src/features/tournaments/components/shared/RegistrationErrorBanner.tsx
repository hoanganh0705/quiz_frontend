...
/**
 * `RegistrationErrorBanner` — error banner for tournament registration/withdrawal errors.
 *
 * Source epic:   Epic 5.1 — SDK coverage & realtime contract foundation.
 * Source story:  5.3 — Tournament registration and participant-state mutations.
 * Source ticket: TKT-5.3.C2.
 *
 * ## Purpose
 *
 * Renders a dismissible error banner with code-specific messaging for all
 * 9 documented registration error codes. Uses `getUserCopy` from Epic 5.1 D3
 * for user-facing copy.
 *
 * ## Variant semantics
 *
 * - `ALREADY_REGISTERED` renders an informational (blue/info) banner because
 *   it is not a user error — the user is already registered.
 * - All other error codes render a destructive (red/error) banner.
...
export interface RegistrationErrorBannerProps {
  /** The error to display. `null` renders nothing. */
  error: ApiError | null;
...
/**
 * Check if an error code is informational (not a destructive error).
 *
 * `ALREADY_REGISTERED` is informational because it indicates the user
 * is already registered, not that something went wrong.
 */
...
/**
 * The registration/withdrawal error banner.
 */
