...
// Allow other components (e.g. the header search button) to open QuickSearch
// without exposing its internal `open` state. The button dispatches a
// `quick-search:open` window event; we ignore duplicates within 200ms so
// accidental double-clicks don't bounce the dialog.
useEffect(() => {
  let lastOpen = 0
  const handler = () => {
    const now = Date.now()
  ...
  window.addEventListener('quick-search:open', handler)
  return () => window.removeEventListener('quick-search:open', handler)
}, [])
