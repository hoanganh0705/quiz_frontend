/**
 * `trending-route.spec.tsx` — Locks the `/social/users/trending` route contract
 * (TKT-6.5.G1).
 */
...
import TrendingRoute from "@/app/(protected)/social/users/trending/page";
...
const mockTrendingUsersList = vi.fn(() => (
  <div data-testid="trending-users-list">Live List</div>
));
vi.mock("@/features/social/lists/TrendingUsersList", () => ({
  TrendingUsersList: () => mockTrendingUsersList(),
}));
...
describe("TrendingRoute", () => {
  describe("placeholder branch", () => {
    it("renders SocialDiscoveryPlaceholder when flag is placeholder", () => {
      ...
      render(<TrendingRoute />);

      expect(screen.getByTestId("social-discovery-placeholder")).toBeInTheDocument();
      expect(mockTrendingUsersList).not.toHaveBeenCalled();
    });
  });

  describe("live branch", () => {
    it("renders TrendingUsersList when flag is live and user is authenticated", () => {
  ...
  describe("auth handling", () => {
    ...
    it("renders loading state while auth is loading", () => {
      ...
      const { container } = render(<TrendingRoute />);
