/**
 * `app/admin/tournaments/__tests__/page.spec.tsx`
 *
 * Source epic:   Epic 7.7 — Tournament Admin: Create, Update, Delete.
 * Source ticket: TKT-7.7.F2 (route-level wiring).
 *
 * Validates that the `/admin/tournaments` route file:
 *   1. Renders without crashing.
 *   2. Emits a `tournament-admin.mount` breadcrumb on mount.
 *   3. Renders the documented disabled notice when the
 *      `phase7_admin_tournament` flag is `'placeholder'`.
 *   4. Delegates to `TournamentAdminPage` when the flag is `'enabled'`.
...
describe('AdminTournamentsPage', () => {
  ...
  it('reads the phase7_admin_tournament flag', () => {
    renderPage();
