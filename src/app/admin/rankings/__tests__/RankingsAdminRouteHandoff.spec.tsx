...
describe('RankingsAdminRouteHandoff', () => {
  it('renders disabled notice when flag is placeholder', () => {
    ...
    expect(
      screen.getByText(/phase7_admin_ranking/),
    ).toBeInTheDocument();
  });
  ...
  it('usesAdminFeatureFlag is called with phase7_admin_ranking', async () => {
    setup('placeholder');
    render(<RankingsAdminRouteHandoff />);
    expect(useAdminFeatureFlag).toHaveBeenCalledWith('phase7_admin_ranking');
  });
  ...
  it('renders correct description text for placeholder state', () => {
    ...
    // <code>phase7_admin_ranking</code> sits inside the sentence.
    ...
    expect(fullText).toContain('phase7_admin_ranking');
